 ...
 *
 *
 * double ChooseTimestep(const double p_Age, const DBL_VECTOR& p_tScales)
 *
 * @param       p_Age                           Age of the star (Myr)
 * @return                                      Suggested timestep (Myr)
 */
GNU_CONST double NS::ChooseTimestep(const double p_Age) const {

    double dt = 500.0;                                      // default value
        ...
        const double slope = 1.58859191006;                 // 1.58859191006 = log10(500.0) / (log10(500.0) - 1.0)
        const double step  = slope * (std::log10(p_Age) - 1.0);

        dt = PPOW(10.0, step);
    }

    return std::max(dt, NUCLEAR_MINIMUM_TIMESTEP);          // clamp to minimum NUCLEAR_MINIMUM_TIMESTEP
}


/*
 * CalculateRadiusInKM_Static
 *
 * @brief
 * Calculate Neutron Star radius per user-supplied NS equation-of-state
 * (see option --neutron-star-equation-of-state)
 * 
 * 
 * double CalculateRadiusInKM_Static(const double p_Mass)
 *
 * @param       p_Mass                          Mass of the star (Msol)
 * @return                                      Radius of the star (km)
 */
double NS::CalculateRadiusInKM_Static(const double p_Mass) {
    ...
    switch (OPTIONS->NeutronStarEquationOfState()) {                            // which equation-of-state?
        ...
        case NS_EOS::ARP3: {                                                    // ARP3
            ...
            radius = p_Mass < minMass ? radiusAtMin : (p_Mass > maxMass ? radiusAtMax : utils::SampleFromTabulatedCDF(p_Mass, ARP3_MASS_RADIUS_RELATION))
        } break;
...
double NS::CalculateBirthMagneticField() {
    ...
    switch (OPTIONS->PulsarBirthMagneticFieldDistribution()) {                                                  // which distribution?
        ...
        case PULSAR_BIRTH_MAGNETIC_FIELD_DISTRIBUTION::LOGNORMAL: {                                             // LOG NORMAL distribution from Faucher-Giguere and Kaspi 2006 https://arxiv.org/abs/astro-ph/0512585
            ...
            SizeT iterations = 0;
            do { log10B = RAND->RandomGaussian(sigma) + mean;} while (iterations++ < PULSAR_MAG_ITERATIONS && utils::Compare(log10B, std::log10(NS::NS_MAG_FIELD_LOWER_LIMIT)) <= 0);
...
double NS::CalculateSpinDownRate(const double p_Period, const double p_MomentOfInteria, const double p_MagField, const double p_Radius) const {
   ...
   const double cgsRadius   = p_Radius * KM_TO_CM;                      // radius in cm
   const double radius_6    = cgsRadius * cgsRadius * cgsRadius * cgsRadius * cgsRadius * cgsRadius;
   const double magField_2  = p_MagField * p_MagField;
   constexpr double _8_PI_2 = 8.0 * PI_2;
   constexpr double _3_C_3  = 3.0E6 * C * C * C;                        // 3.0 * (C * 100.0) * (C * 100.0) * (C * 100.0)
   const double pDotTop     = _8_PI_2 * radius_6 * magField_2;
   const double pDotBottom  = _3_C_3 * p_MomentOfInteria * p_Period;
   
   return pDotTop / pDotBottom;                                         // period derivative                                                                          
}
...
void NS::SpinDownIsolatedPulsar(const double p_Stepsize) {
    ...
    const double radius_IN_CM      = m_Radius * RSOL_TO_KM * KM_TO_CM;
    const double radius_3          = radius_IN_CM * radius_IN_CM * radius_IN_CM;
    const double radius_6          = radius_3 * radius_3;
    constexpr double _8_PI_2       = 8.0 * PI_2;
    constexpr double _3_C_3        = 3.0E6 * C * C * C;                                     // 3.0 * (C * 100.0) * (C * 100.0) * (C * 100.0)
    const double initialMagField   = m_PulsarDetails.magneticField;     
    const double initialSpinPeriod = m_PulsarDetails.spinPeriod;
    ...
    if (utils::Compare(initialMagField, NS::NS_MAG_FIELD_LOWER_LIMIT) < 0) {
        ...
        // update pulsar magnetic field in cgs
        m_PulsarDetails.magneticField = NS::NS_MAG_FIELD_LOWER_LIMIT + (initialMagField - NS::NS_MAG_FIELD_LOWER_LIMIT) * std::exp(-p_Stepsize / NS::NS_DECAY_TIME_SCALE);
    }
    // Calculate the spin down rate for isolated neutron stars (see Eq 3 in arxiv:2406.11428)
    // Note that magnetic and rotational axes are orthogonal, leading to sin^2(alpha) = 1 in this equation. 
    // The rest of the calculations are carried out in cgs.   
    const double constant2        = (_8_PI_2 * radius_6) / (_3_C_3 * m_MomentOfInertia_CGS);
    const double term1            = NS::NS_MAG_FIELD_LOWER_LIMIT * NS::NS_MAG_FIELD_LOWER_LIMIT * p_Stepsize;
    const double term2            = NS::NS_DECAY_TIME_SCALE * NS::NS_MAG_FIELD_LOWER_LIMIT * ( m_PulsarDetails.magneticField  - initialMagField);
    const double term3            = (NS::NS_DECAY_TIME_SCALE / 2.0) * ((m_PulsarDetails.magneticField * m_PulsarDetails.magneticField) - (initialMagField * initialMagField));
    const double Psquared         = 2.0 * constant2 * (term1 - term2 - term3) + (initialSpinPeriod * initialSpinPeriod);
...
double NS::DeltaJByAccretion_Static(const double p_Mass, const double p_Radius_6, const double p_MagField, const double p_SpinFrequency, const double p_mDot, const double p_Epsilon)  {
    ...
    const double p              = p_Radius_6 * p_Radius_6 / (p_mDot * p_mDot * p_Mass);
...
void NS::UpdateMagneticFieldAndSpin(const bool p_CommonEnvelope, const bool p_RecycledNS, double p_Stepsize, double p_MassGain, const double p_Epsilon) {
    ...
    if ((!p_RecycledNS && !p_CommonEnvelope) || (!p_RecycledNS && utils::Compare(p_MassGain, 0.0) == 0 )) {                     // 'classical' isolated pulsars
        ...
        const double massG     = m_Mass * MSOL_TO_G;                                                                            // mass in g
        const double radiusCM  = m_Radius * RSOL_TO_CM;                                                                         // radius in cm
        ...
        const double initialAngularMomentum_CGS = m_AngularMomentum_CGS;                                                        // initial angular momentum
        const double initialMagField            = m_PulsarDetails.magneticField;                                                // initial magnetic field
        const double initialMass                = m_Mass * MSOL_TO_G;                                                           // initial mass of NS in g

        const double radius                     = m_Radius * RSOL_TO_CM;                                                        // radius of NS in cm
        const double radius_6                   = radius * radius * radius * radius * radius * radius;                          // for performance - do it once
        const double mDot                       = p_MassGain / p_Stepsize;                                                      // required mass transfer rate (g s^-1)
        const double massFinal                  = initialMass + p_MassGain;                                                     // required final mass of NS (after accretion) in g


        // calculate initial mass slice size for integration
        const double jAcc      = DeltaJByAccretion_Static(initialMass, radius_6, m_PulsarDetails.magneticField, m_PulsarDetails.spinFrequency, mDot, p_Epsilon);
        const double massSlice = std::fabs(m_AngularMomentum_CGS / 1000.0 / jAcc);                                              // abs(Jx10^-3 / dJdM)
        ...
        struct ode {
            ...
            void operator () (const state_type& x, state_type& dxdm, double p_MassDelta ) const {
                ...
                const double B = (p_MagField - NS::NS_MAG_FIELD_LOWER_LIMIT) * std::exp(-p_MassDelta / NS::NS_DECAY_MASS_SCALE) + NS::NS_MAG_FIELD_LOWER_LIMIT;
                const double f = x[0] / CalculateMomentOfInertiaCGS_Static(m, p_Radius);
        ...
        m_PulsarDetails.spinFrequency = m_AngularMomentum_CGS / m_MomentOfInertia_CGS;
        m_PulsarDetails.spinPeriod    = _2_PI / m_PulsarDetails.spinFrequency;
        const double fDot             = (m_AngularMomentum_CGS - initialAngularMomentum_CGS) / m_MomentOfInertia_CGS / p_Stepsize; // eq. 11 in arxiv:1912.02415 
        m_PulsarDetails.spinDownRate  = -fDot * m_PulsarDetails.spinPeriod * m_PulsarDetails.spinPeriod / _2_PI;
    }      
    else {                                                                                                                      // otherwise...    
