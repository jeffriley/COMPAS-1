...
//                   OPTIONS->OptionSpecified("metallicity") == 0 ||   // for now we don't sample metallicity - we always return ZSOL
    ...
    do {
        ...
        // the initial stellar type of the stars may be supplied - this is to allow the user to 
        // specify where evolution should start

        // create stars according to the user-specified options
        if (OPTIONS->OptionSpecified("initial-stellar-type-1") == 1 && OPTIONS->InitialStellarType1() != STELLAR_TYPE::NONE) {          // user specified valid initial stellar type for primary?                   
            m_Star1 = new BinaryConstituentStar(m_RandomSeed, OPTIONS->InitialStellarType1(), mass1, metallicity, kickParameters1);     // yes - use it
        }
        else {
            m_Star1 = new BinaryConstituentStar(m_RandomSeed, mass1, metallicity, kickParameters1);                                     // no - initial stellar type wil be one of the MS variants depending upon initial mass
        }

        if (OPTIONS->OptionSpecified("initial-stellar-type-2") == 1 && OPTIONS->InitialStellarType2() != STELLAR_TYPE::NONE) {          // user specified valid initial stellar type for secondary?                   
            m_Star2 = new BinaryConstituentStar(m_RandomSeed, OPTIONS->InitialStellarType2(), mass2, metallicity, kickParameters2);     // yes - use it
        }
        else {
            m_Star2 = new BinaryConstituentStar(m_RandomSeed, mass2, metallicity, kickParameters2);                                     // no - initial stellar type wil be one of the MS variants depending upon initial mass
        }
