...
private:
    ...
    std::vector<std::tuple<std::string, std::string, std::string, bool, std::string>> deprecatedOptionValues = {
        ...
        { "mode",                                 "SSE",         "SSE_HURLEY",  false, "20270601" },
        { "mode",                                 "BSE",         "BSE_HURLEY",  false, "20270601" }
    };
...
public:
    ...
    class OptionValues {
        ...
        private:
            ...
            bool                                                m_ScaleCHEMassLossWithSurfaceHeliumAbundance;                   // Whether to transition between OB and WR mass loss rates for CH stars on the MS
            ...
            ENUM_OPT<LBV_ML_PRESCRIPTION>                m_LBVMassLossPrescription;                                      // Which LBV mass loss prescription to use
            ...
            ENUM_OPT<OB_ML_PRESCRIPTION>                 m_OBMassLossPrescription;                                       // Which OB mass loss prescrioption
            ENUM_OPT<VMS_ML_PRESCRIPTION>                m_VMSMassLossPrescription;                                      // Which VMS mass loss prescription for M > 100 Msol        
            ENUM_OPT<RSG_ML_PRESCRIPTION>                m_RSGMassLossPrescription;                                      // Which RSG mass loss prescription to use for RSG       
            ENUM_OPT<WR_ML_PRESCRIPTION>                 m_WRMassLossPrescription;                                       // Which WR mass loss prescription to use for WR       
            ...
            ENUM_OPT<MS_CORE_MASS_PRESCRIPTION>                 m_MainSequenceCoreMassPrescription;                             // Which MS core prescription
            ...
            double                                              m_Metallicity1;                                                 // Metallicity of the primary star (BSE); if not specified, falls back to --metallicity then default
            double                                              m_Metallicity2;                                                 // Metallicity of the secondary star (BSE); if not specified, falls back to --metallicity then default
            unsigned int                                        m_MetallicityCacheSize;                                         // ZDependent cache size
...
private:
    ...
    GridfileT                   m_Gridfile = {"", ERROR::EMPTY_FILENAME, {}, 0, 0, 0, 0};   // explicit init for handle/startLine/currentLine/linesProcessed/linesToProcess - silences -Wmissing-field-initializers
...
public:
    ...
    EVOLUTION_MODE                              Mode() const                                                            { return EvolutionMode(); }                                      // succinct alias for EvolutionMode()
    ...
    double                                      Fryer22fmix() const                                                     { return OPT_VALUE("fryer-22-fmix", m_Fryer2022fmix, true); }
    double                                      Fryer22Mcrit() const                                                    { return OPT_VALUE("fryer-22-mcrit", m_Fryer2022Mcrit, true); }
    ...
    std::string                                 LogfileDetailedOutput() const                                           { return m_CmdLine.optionValues.m_Populated && !m_CmdLine.optionValues.m_VM["logfile-detailed-output"].defaulted()
                                                                                                                                    ...
                                                                                                                                    : (m_CmdLine.optionValues.m_EvolutionMode.type == EVOLUTION_MODE::SSE_HURLEY
    ...
    LBV_ML_PRESCRIPTION                  LBVMassLossPrescription() const                                         { return OPT_VALUE("LBV-mass-loss-prescription", m_LBVMassLossPrescription.type, true); }
    ...
    double                                      Metallicity1() const                                                    { return OPT_DEFAULTED("metallicity-1") ? Metallicity() : OPT_VALUE("metallicity-1", m_Metallicity1, true); }   // primary (BSE): --metallicity-1 -> --metallicity -> default
    double                                      Metallicity2() const                                                    { return OPT_DEFAULTED("metallicity-2") ? Metallicity() : OPT_VALUE("metallicity-2", m_Metallicity2, true); }   // secondary (BSE): --metallicity-2 -> --metallicity -> default
    ...
    OB_ML_PRESCRIPTION                   OBMassLossPrescription() const                                          { return OPT_VALUE("OB-mass-loss-prescription", m_OBMassLossPrescription.type, true); }
    ...
    RSG_ML_PRESCRIPTION                  RSGMassLossPrescription() const                                         { return OPT_VALUE("RSG-mass-loss-prescription", m_RSGMassLossPrescription.type, true); }
    ...
    VMS_ML_PRESCRIPTION                  VMSMassLossPrescription() const                                         { return OPT_VALUE("VMS-mass-loss-prescription", m_VMSMassLossPrescription.type, true); }
    double                                      WolfRayetFactor() const                                                 { return OPT_VALUE("wolf-rayet-multiplier", m_WolfRayetFactor, true); }
    WR_ML_PRESCRIPTION                   WRMassLossPrescription() const                                          { return OPT_VALUE("WR-mass-loss-prescription", m_WRMassLossPrescription.type, true); }
