...
void Options::OptionValues::Initialise() {
    ...
    m_LBVMassLossPrescription.type                                  = LBV_ML_PRESCRIPTION::HURLEY_ADD;
    m_LBVMassLossPrescription.typeString                            = LBV_ML_PRESCRIPTION_LABEL.at(m_LBVMassLossPrescription.type);

    m_OBMassLossPrescription.type                                   = OB_ML_PRESCRIPTION::VINK2021;
    m_OBMassLossPrescription.typeString                             = OB_ML_PRESCRIPTION_LABEL.at(m_OBMassLossPrescription.type);

    m_VMSMassLossPrescription.type                                  = VMS_ML_PRESCRIPTION::SABHAHIT2023;
    m_VMSMassLossPrescription.typeString                            = VMS_ML_PRESCRIPTION_LABEL.at(m_VMSMassLossPrescription.type);

    m_RSGMassLossPrescription.type                                  = RSG_ML_PRESCRIPTION::DECIN2023;
    m_RSGMassLossPrescription.typeString                            = RSG_ML_PRESCRIPTION_LABEL.at(m_RSGMassLossPrescription.type);

    m_WRMassLossPrescription.type                                   = WR_ML_PRESCRIPTION::SANDERVINK2023;
    m_WRMassLossPrescription.typeString                             = WR_ML_PRESCRIPTION_LABEL.at(m_WRMassLossPrescription.type);
    ...
    m_Metallicity1                                                  = ZSOL_ASPLUND;
    m_Metallicity2                                                  = ZSOL_ASPLUND;
    m_MetallicityCacheSize                                          = 8;
...
bool Options::AddOptions(OptionValues *p_Options, po::options_description *p_OptionsDescription) {
    ...
    try {
        ...
        (
            "metallicity-cache-size",
            po::value<unsigned int>(&p_Options->m_MetallicityCacheSize)->default_value(p_Options->m_MetallicityCacheSize),
            ("ZDependent metallicity cache size, 1..64 (default = " + std::to_string(p_Options->m_MetallicityCacheSize) + "; <=8 uses linear scan, >8 uses hash backend)").c_str()
        )
        ...
        (
            "metallicity-1",
            po::value<double>(&p_Options->m_Metallicity1)->default_value(p_Options->m_Metallicity1),
            ("Metallicity for the primary star (BSE); if not specified, --metallicity is used (default = " + std::to_string(p_Options->m_Metallicity1) + ")").c_str()
        )
        (
            "metallicity-2",
            po::value<double>(&p_Options->m_Metallicity2)->default_value(p_Options->m_Metallicity2),
            ("Metallicity for the secondary star (BSE); if not specified, --metallicity is used (default = " + std::to_string(p_Options->m_Metallicity2) + ")").c_str()
        )
        (
            "metallicity-max",                                            
...
#define DEFAULTED(opt) m_VM[opt].defaulted()
    ...
    try {
        ...
        if (!DEFAULTED("LBV-mass-loss-prescription")) {                                                                             // LBV mass loss prescription
            std::tie(found, m_LBVMassLossPrescription.type) = utils::GetMapKey(m_LBVMassLossPrescription.typeString, LBV_ML_PRESCRIPTION_LABEL, m_LBVMassLossPrescription.type);
        ...
        if (!DEFAULTED("OB-mass-loss-prescription")) {                                                                              // OB (main sequence) mass loss prescription
            std::tie(found, m_OBMassLossPrescription.type) = utils::GetMapKey(m_OBMassLossPrescription.typeString, OB_ML_PRESCRIPTION_LABEL, m_OBMassLossPrescription.type);
        ...
        if (!DEFAULTED("RSG-mass-loss-prescription")) {                                                                             // RSG mass loss prescription
            std::tie(found, m_RSGMassLossPrescription.type) = utils::GetMapKey(m_RSGMassLossPrescription.typeString, RSG_ML_PRESCRIPTION_LABEL, m_RSGMassLossPrescription.type);
        ...
        if (!DEFAULTED("VMS-mass-loss-prescription")) {                                                                             // very massive (VMS) mass loss prescription
            std::tie(found, m_VMSMassLossPrescription.type) = utils::GetMapKey(m_VMSMassLossPrescription.typeString, VMS_ML_PRESCRIPTION_LABEL, m_VMSMassLossPrescription.type);
        ...
        if (!DEFAULTED("WR-mass-loss-prescription")) {                                                                              // WR mass loss prescription
            std::tie(found, m_WRMassLossPrescription.type) = utils::GetMapKey(m_WRMassLossPrescription.typeString, WR_ML_PRESCRIPTION_LABEL, m_WRMassLossPrescription.type);
        ...
        COMPLAIN_IF(m_Metallicity1 < MINIMUM_METALLICITY || m_Metallicity1 > MAXIMUM_METALLICITY, "Metallicity (--metallicity-1) should be absolute metallicity and must be between " + std::to_string(MINIMUM_METALLICITY) + " and " + std::to_string(MAXIMUM_METALLICITY));
        COMPLAIN_IF(m_Metallicity2 < MINIMUM_METALLICITY || m_Metallicity2 > MAXIMUM_METALLICITY, "Metallicity (--metallicity-2) should be absolute metallicity and must be between " + std::to_string(MINIMUM_METALLICITY) + " and " + std::to_string(MAXIMUM_METALLICITY));
        COMPLAIN_IF(m_MetallicityCacheSize < 1 || m_MetallicityCacheSize > 64, "Metallicity cache size (--metallicity-cache-size) must be between 1 and 64");
        ...
        COMPLAIN_IF(m_ObjectsToEvolve <= 0, (m_EvolutionMode.type == EVOLUTION_MODE::SSE_HURLEY ? "Number of stars requested <= 0" : "Number of binaries requested <= 0"));
...
#define POPULATE_RET(mapname) for (auto& it: mapname) ret.push_back("'" + it.second + "'")
    ...
    switch (_(p_OptionString.c_str())) {    // which option?
        ...
        case _("LBV-mass-loss-prescription")                        : POPULATE_RET(LBV_ML_PRESCRIPTION_LABEL);               break;
        ...
        case _("OB-mass-loss-prescription")                         : POPULATE_RET(OB_ML_PRESCRIPTION_LABEL);                break;
        ...
        case _("RSG-mass-loss-prescription")                        : POPULATE_RET(RSG_ML_PRESCRIPTION_LABEL);               break;
        ...
        case _("VMS-mass-loss-prescription")                        : POPULATE_RET(VMS_ML_PRESCRIPTION_LABEL);               break;
        case _("WR-mass-loss-prescription")                         : POPULATE_RET(WR_ML_PRESCRIPTION_LABEL);                break;
        default: break;
    }
    ...
    try {
        ...
        if (errStr.empty()) {                                                                                               // no need if we've already flagged an error
            ...
            if (removeCount > 0) {                                                                                          // anything to remove?
                for (size_t iOpt = removeCount; iOpt-- > 0; ) {                                                             // loop through all complex options identified to be removed (post-dec avoids size_t underflow)
                ...
                if (it != p_OptionsDescriptor.optionValues.m_VM.end()) {                                                    // found?
                    ...
                    if (type == COMPLEX_TYPE::RANGE) {                                                                      // RANGE?
                        ...
                        if (IsSupportedNumericDataType(dataType)) {                                                         // numeric?
                            ...
                            for (size_t ip = 0; ip < parms.size(); ip++) {                                                  // yes - for each set parameter specified
                                int check = std::abs(static_cast<long>(utils::IsBOOL(parms[ip])));                                             // check param for valid BOOL
                                if (check == 0) break;
                                checks[check - 1]++;
                            }
...
COMPAS_VARIABLE Options::OptionValue(const T_ANY_PROPERTY p_Property) const {
    ...
    switch (property) {
        ...
        case PROGRAM_OPTION::LBV_ML_PRESCRIPTION                     : value = static_cast<int>(LBVMassLossPrescription());                          break;
