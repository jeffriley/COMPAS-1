...
using DblArrayT         = std::array<double, Size>;
template <std::size_t Size>
using OptDblArrayT      = std::array<OptDblT, Size>;

using ObjectIdT         = ULongT;

using HurleyACoeffsT      = DblArrayT<A_COEFF.size() + 1>;  // Indexing is 1-based
using HurleyBCoeffsT      = DblArrayT<B_COEFF.size() + 1>;  // Indexing is 1-based
using HurleyAlphasT       = DblArrayT<3>;   // See Hurley et al., 2000 (alpha1, alpha2, and alpha4 only!  See ZDependent)
using HurleyGConstsT      = DblArrayT<HURLEY_GAMMA_CONSTS::COUNT>;
using HurleyLConstsT      = DblArrayT<HURLEY_L_CONSTANTS::COUNT>;
using HurleyRConstsT      = DblArrayT<HURLEY_R_CONSTANTS::COUNT>;
using HurleyMassCutoffsT  = DblArrayT<HURLEY_MASS_CUTOFFS::COUNT>;

using TimescalesT         = OptDblArrayT<TIMESCALES_COUNT>;
using GBParamsT           = OptDblArrayT<GBPARAMS_COUNT>;

using UnpackedTimescalesT = DblArrayT<TIMESCALES_COUNT>;
using UnpackedGBParamsT   = DblArrayT<GBPARAMS_COUNT>;

using ToutLCoeffsT        = DblArrayT<L_COEFF.size()>;
using ToutRCoeffsT        = DblArrayT<R_COEFF.size()>;

using ShikauchiACoeffsT    = DblArrayT<SHIKAUCHI_A_COEFF_COUNT>;
using ShikauchiFCoeffsT    = DblArrayT<SHIKAUCHI_F_COEFF_COUNT>;
using ShikauchiLCoeffsT    = DblArrayT<SHIKAUCHI_L_COEFF_COUNT>;

using Vector3dT           = DblArrayT<3>; // exactly 3 doubles - enforced at compile time
using Matrix3x3T          = std::array<Vector3dT, 3>; // 3x3 matrix
...
// starting stellar type (for SSE/BSE option parsing - mirrors STELLAR_TYPE with Hurley numbering)
enum class STARTING_STELLAR_TYPE: int { // Hurley
    NONE = -1,                          // 
    ms = 0,                             //   0 - preserves Hurley numbering - resolves to MS
    MS,                                 //   1
    HG,                                 //   2
    FGB,                                //   3
    CHeB,                               //   4
...
enum class LBV_ML_PRESCRIPTION: int { ZERO, HURLEY_ADD, HURLEY, BELCZYNSKI };
const COMPASUnorderedMap<LBV_ML_PRESCRIPTION, std::string> LBV_ML_PRESCRIPTION_LABEL = {
    { LBV_ML_PRESCRIPTION::ZERO,       "ZERO" },
    { LBV_ML_PRESCRIPTION::HURLEY_ADD, "HURLEY_ADD" },
    { LBV_ML_PRESCRIPTION::HURLEY,     "HURLEY" },
    { LBV_ML_PRESCRIPTION::BELCZYNSKI, "BELCZYNSKI" }
};
...
enum class ML_TYPE: int { NONE, GB, LBV, OB, RSG, VMS, WR};
const COMPASUnorderedMap<ML_TYPE, std::string> ML_TYPE_LABEL = {
    { ML_TYPE::NONE, "NONE" },
    { ML_TYPE::GB,   "GB" },
    { ML_TYPE::LBV,  "LBV" },
    { ML_TYPE::OB,   "OB" },
...
enum class OB_ML_PRESCRIPTION: int { ZERO, VINK2001, VINK2021, BJORKLUND2022, KRTICKA2018};
const COMPASUnorderedMap<OB_ML_PRESCRIPTION, std::string> OB_ML_PRESCRIPTION_LABEL = {
    { OB_ML_PRESCRIPTION::ZERO,          "ZERO" },
    { OB_ML_PRESCRIPTION::VINK2001,      "VINK2001" },
    { OB_ML_PRESCRIPTION::VINK2021,      "VINK2021" },
    { OB_ML_PRESCRIPTION::BJORKLUND2022, "BJORKLUND2022" },
    { OB_ML_PRESCRIPTION::KRTICKA2018,   "KRTICKA2018" }
};
...
// stellar populations
enum class STELLAR_POPULATION: int { POPULATION_I, POPULATION_II }; // <<<<<<<<<<<<<<<<<<<<<< JR FIX THIS popI was 1, popII 0
...
enum class HURLEY_TIMESCALES: int {
    ...
    Inf1_FGB,              // First Giant Branch tinf1
    Inf2_FGB,              // First Giant Branch tinf2
    Mx_FGB,                // First Giant Branch t(Mx)

                            // Early Asymptotic Giant Branch (EAGB) (FAGB in Hurley's sse)
    Inf1_FAGB,             // Early Asymptotic Giant Branch tinf1
    Inf2_FAGB,             // Early Asymptotic Giant Branch tinf2
    Mx_FAGB,               // Early Asymptotic Giant Branch t(Mx)

                            // Thermally Pulsating Asymptotic Giant Branch (TPAGB) (SAGB in Hurley's sse)
    Inf1_SAGB,             // Thermally Pulsating Asymptotic Giant Branch tinf1
    Inf2_SAGB,             // Thermally Pulsating Asymptotic Giant Branch tinf2
    Mx_SAGB,               // Thermally Pulsating Asymptotic Giant Branch t(Mx)
    DU2,                     // (tDU?)
                            // Helium Giant Branch
    HeMS,                  // Naked Helium Star central helium burning lifetime (HeMs)
    Inf1_HeGB,             // Helium Giant Branch tinf1
    Inf2_HeGB,             // Helium Giant Branch tinf2
...
enum class RSG_ML_PRESCRIPTION: int { ZERO, VINKSABHAHIT2023, BEASOR2020, DECIN2023, YANG2023, KEE2021, NJ90};
const COMPASUnorderedMap<RSG_ML_PRESCRIPTION, std::string> RSG_ML_PRESCRIPTION_LABEL = {
    { RSG_ML_PRESCRIPTION::ZERO,             "ZERO" },
    { RSG_ML_PRESCRIPTION::VINKSABHAHIT2023, "VINKSABHAHIT2023" },
    { RSG_ML_PRESCRIPTION::BEASOR2020,       "BEASOR2020" },
    { RSG_ML_PRESCRIPTION::DECIN2023,        "DECIN2023" },
    { RSG_ML_PRESCRIPTION::YANG2023,         "YANG2023" },
    { RSG_ML_PRESCRIPTION::KEE2021,          "KEE2021" },
    { RSG_ML_PRESCRIPTION::NJ90,             "NJ90" }
};

// VMS (very massive stars) mass loss prescriptions
enum class VMS_ML_PRESCRIPTION: int { ZERO, VINK2011, BESTENLEHNER2020, SABHAHIT2023};
const COMPASUnorderedMap<VMS_ML_PRESCRIPTION, std::string> VMS_ML_PRESCRIPTION_LABEL = {
    { VMS_ML_PRESCRIPTION::ZERO,             "ZERO" },
    { VMS_ML_PRESCRIPTION::VINK2011,         "VINK2011" },
    { VMS_ML_PRESCRIPTION::BESTENLEHNER2020, "BESTENLEHNER2020" },
    { VMS_ML_PRESCRIPTION::SABHAHIT2023,     "SABHAHIT2023" }
};

// WR mass loss prescriptions
enum class WR_ML_PRESCRIPTION: int { ZERO, BELCZYNSKI2010, SANDERVINK2023, SHENAR2019 };
const COMPASUnorderedMap<WR_ML_PRESCRIPTION, std::string> WR_ML_PRESCRIPTION_LABEL = {
    { WR_ML_PRESCRIPTION::ZERO,           "ZERO"},
    { WR_ML_PRESCRIPTION::BELCZYNSKI2010, "BELCZYNSKI2010" },
    { WR_ML_PRESCRIPTION::SANDERVINK2023, "SANDERVINK2023" },
    { WR_ML_PRESCRIPTION::SHENAR2019,     "SHENAR2019" }
};
...
// Option details
struct OptionDetailsT {
    StrT        optionStr;                                  // name string
    StrT        valueStr;                                   // value string
    StrT        sourceStr;                                  // source string (COMPAS DEFAULT or USER SUPPLIED)    
    StrT        typeStr;                                    // detailed data type (e.g. UNSIGNED LONG INT)
    TYPENAME    dataType;                                   // short data type (e.g. INT, FLOAT, etc)
    StrT        defaultStr;                                 // default value string
    StrVectorT  allowedStr;                                 // vector of allowed value strings (empty for options that don't have multiple allowed values)
};


// Log file details
struct LogfileDetailsT {
    int                           id;                       // logfile id
    StrT                          filename;                 // filename
    ...
    StrVectorT                    hdrStrings;               // the column header strings
    StrVectorT                    unitsStrings;             // the column units strings
    StrVectorT                    typeStrings;              // the column datatype strings
    StrVectorT                    fmtStrings;               // format strings for the columns - how the value is formatted for printing to the logfile
    BoolVectorT                   annotations;              // print flags for each annotation specified by the user (e.g. OPTIONS->NotesHdrs())
};
...
struct RotationalVelocityParamsT {                           // Structure containing parameter (u) for the root solving function using gsl_root_solver
    double u;                                               // Value of CDF, draw in U(0,1)
};


// KickMagnitudeParams struct for gsl root solver
struct KickMagnitudeParamsT {
...
typedef struct KickParametersT {
...
struct SupernovaDetailsT {                           // Holds attributes, flags - if the star went supernova   
...
struct PulsarDetailsT {
    double angularMomentum;                                 // Angular momentum (g cm^2 s^-1)
    double birthPeriod;                                     // Birth period (s)
    double birthSpinDownRate;                               // Birth spin down rate as Pdot (s s^-1)
    double magneticField;                                   // Magnetic field strength (G)
    double momentOfInertia;                                 // Moment of interia (g cm^2)
    double spinDownRate;                                    // Spin down rate as time derivative of spin frequency (fdot, rad s^-2)
    double spinFrequency;                                   // Spin frequency in rads per second
    double spinPeriod;                                      // Spin period (ms)
};

struct PulsarConstants {
    double baryonicMassOfMaxMassNS;                         // Baryonic mass for which the gravitational remnant mass equals the max NS mass (see OPTIONS->MaximumNeutronStarMass())
    double decayMassScale;                                  // Magnetic fielf decay mass scale (see OPTIONS->PulsarMagneticFieldDecayMassscale())
    double decayTimeScale;                                  // Magnetic fielf decay time scale  ( OPTIONS->PulsarMagneticFieldDecayTimescale()see )
    double minMagField;                                     // magnetic field lower limit (10^OPTIONS->PulsarLog10MinimumMagneticField()) 
    double log10MinMagField;                                // log10(minimum magnetic field) (see OPTIONS->PulsarLog10MinimumMagneticField())
