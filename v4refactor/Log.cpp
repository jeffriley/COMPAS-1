...
// Singleton state flag. See Log.h for usage and IsInstantiated() accessor.
bool Log::m_Instantiated = false;
...
bool Log::OpenHDF5RunDetailsFile(const string p_Filename) {
    ...
    if (!m_Files.Enabled) return m_Files.Enabled;                                                                                   // logging not enabled - no business being here
    ...
    m_Hdf5.RunDetailsFile.fileId = m_Hdf5.ContainerId;                                                                   // record run details HDF5 fileid - just the HDF5 container id
    ...
    hid_t h5GroupId    = H5Gopen(m_Hdf5.RunDetailsFile.fileId, h5GroupName.c_str(), H5P_DEFAULT);                       // open the group
    if (h5GroupId >= 0) {                                                                                               // group open (and therefore already exists)?
        ...
        h5GroupId = H5Gcreate(m_Hdf5.ContainerId, h5GroupName.c_str(), 0, H5P_DEFAULT, H5P_DEFAULT);                     // create the group
        if (h5GroupId < 0) {                                                                                            // group created ok?
            ...
            m_Hdf5.RunDetailsFile.groupId = h5GroupId;                                                                  // record group id for run details file
            ...
            if (ok) {                                                                                                   // still ok?
                ...
                m_Hdf5.RunDetailsFile.chunkSize = chunkSize;                                                            // record chunk size for file
                m_Hdf5.RunDetailsFile.IOBufSize = IOBufSize;                                                            // record IO buf size for file
                ...
                for (int dSetIdx = static_cast<int>(RUN_DETAILS_COLUMNS::COMPAS_VERSION); dSetIdx < static_cast<int>(RUN_DETAILS_COLUMNS::SENTINEL); dSetIdx++ ) {
                    ...
                    try { runDetails = RUN_DETAILS_DETAIL.at(static_cast<RUN_DETAILS_COLUMNS>(dSetIdx)); }              // get run details details
                        ...
                        Squawk("ERROR: Unknown property for HDF5 file with name " + m_Hdf5.ContainerName);                        // announce error
                        ok = false;                                                                                     // fail
                    }
                    ...
                    if (ok) {                                                                                           // have valid property
                        ...
                        h5Dset = CreateHDF5Dataset(m_Hdf5.ContainerName, h5GroupId, h5DatasetName, h5DataType, "-", chunkSize); // create dataset
                        if (h5Dset < 0) {                                                                               // dataset not created
                            ...
                            m_Hdf5.RunDetailsFile.dataSets.push_back({h5Dset, h5DataType, compasType, STRING_QUALIFIER::FIXED_LENGTH, {}}); // record dataset details

                            // derivation
                            h5DatasetName += "-Derivation";                                                             // derivation
                            h5Dset         = CreateHDF5Dataset(m_Hdf5.ContainerName, h5GroupId, h5DatasetName, h5String13DataType, "-", chunkSize); // create dataset
                            if (h5Dset < 0) {                                                                           // dataset not created
                                Squawk("ERROR: Error creating HDF5 dataset with name " + h5DatasetName);                // announce error
                                ok = false;                                                                             // fail
                            }
                            else
                                m_Hdf5.RunDetailsFile.dataSets.push_back({h5Dset, h5String13DataType, TYPENAME::STRING, STRING_QUALIFIER::FIXED_LENGTH, {}}); // dataset created ok - record details
                        }
                    }
                ...
                for (std::size_t idx = 0; idx < m_Config.OptionDetails.size(); idx++) {                                        // for each program option
                    // option
                    TYPENAME compasType = m_Config.OptionDetails[idx].dataType;                                                // COMPAS data type
                    h5DataType          = GetHDF5DataType(compasType, (m_Config.OptionDetails[idx].valueStr).length());        // HDF5 data type for COMPAS data type
                    h5DatasetName       = m_Config.OptionDetails[idx].optionStr;                                               // dataset (option name)
                    h5Dset              = CreateHDF5Dataset(m_Hdf5.ContainerName, h5GroupId, h5DatasetName, h5DataType, "-", chunkSize); // create dataset
                    if (h5Dset < 0) {                                                                                   // dataset not created
                        Squawk("ERROR: Error creating HDF5 dataset with name " + h5DatasetName);                        // announce error
                        ok = false;                                                                                     // fail
...
void Log::Start(const string      p_LogBasePathString,
    ...
    if (!m_Files.Enabled) {                                                                                                       // logging enabled?
                                                                                                                            // no...
        // start timers etc.
        m_Files.WallStart         = std::chrono::system_clock::now();                                                             // start wall timer
        m_Files.ClockStart        = clock();                                                                                      // start CPU timer

        // enable logging
        m_Files.Enabled           = true;                                                                                         // logging enabled
        m_Config.LogBasePathString = p_LogBasePathString;                                                                          // set base path
        m_Config.LogNamePrefix     = p_LogNamePrefix;                                                                              // set log file name prefix
        m_Config.LogLevel          = p_LogLevel;                                                                                   // set log level
        m_Config.LogClasses        = p_LogClasses;                                                                                 // set enabled log classes
        m_Config.DbgLevel          = p_DbgLevel;                                                                                   // set debug level
        m_Config.DbgClasses        = p_DbgClasses;                                                                                 // set enagled debug classes
        m_Config.DbgToLogfile      = p_DbgToLogfile;                                                                               // write debug records to logfile?
        m_Config.ErrToLogfile      = p_ErrorsToLogfile;                                                                            // write error records to logfile?
        m_Config.LogfileType       = p_LogfileType;                                                                                // set log file type

        m_Files.Logfiles.clear();                                                                                                 // clear all entries

        m_Config.OptionDetails = OPTIONS->CmdLineOptionsDetails();                                                                 // get commandline option details
        ...
        // BSE
        if (NotesPropertyPresent(m_Specs.BSE_CEE_Rec         )) m_Specs.BSE_CEE_Notes          = BOOL_VECTOR(m_Specs.BSE_CEE_Notes.size(), true);
        if (NotesPropertyPresent(m_Specs.BSE_DCO_Rec         )) m_Specs.BSE_DCO_Notes          = BOOL_VECTOR(m_Specs.BSE_DCO_Notes.size(), true);
        if (NotesPropertyPresent(m_Specs.BSE_Detailed_Rec    )) m_Specs.BSE_Detailed_Notes     = BOOL_VECTOR(m_Specs.BSE_Detailed_Notes.size(), true);
        if (NotesPropertyPresent(m_Specs.BSE_Pulsars_Rec     )) m_Specs.BSE_Pulsars_Notes      = BOOL_VECTOR(m_Specs.BSE_Pulsars_Notes.size(), true);
        if (NotesPropertyPresent(m_Specs.BSE_RLOF_Rec        )) m_Specs.BSE_RLOF_Notes         = BOOL_VECTOR(m_Specs.BSE_RLOF_Notes.size(), true);
        if (NotesPropertyPresent(m_Specs.BSE_SNE_Rec         )) m_Specs.BSE_SNE_Notes          = BOOL_VECTOR(m_Specs.BSE_SNE_Notes.size(), true);
        if (NotesPropertyPresent(m_Specs.BSE_Switch_Rec      )) m_Specs.BSE_Switch_Notes       = BOOL_VECTOR(m_Specs.BSE_Switch_Notes.size(), true);
        if (NotesPropertyPresent(m_Specs.BSE_SysParms_Rec    )) m_Specs.BSE_SysParms_Notes     = BOOL_VECTOR(m_Specs.BSE_SysParms_Notes.size(), true);
        if (NotesPropertyPresent(m_Specs.BSE_Sys_Snapshot_Rec)) m_Specs.BSE_Sys_Snapshot_Notes = BOOL_VECTOR(m_Specs.BSE_Sys_Snapshot_Notes.size(), true);

        // SSE
        if (NotesPropertyPresent(m_Specs.SSE_Detailed_Rec    )) m_Specs.SSE_Detailed_Notes     = BOOL_VECTOR(m_Specs.SSE_Detailed_Notes.size(), true);
        if (NotesPropertyPresent(m_Specs.SSE_Pulsars_Rec     )) m_Specs.SSE_Pulsars_Notes      = BOOL_VECTOR(m_Specs.SSE_Pulsars_Notes.size(), true);
        if (NotesPropertyPresent(m_Specs.SSE_SNE_Rec         )) m_Specs.SSE_SNE_Notes          = BOOL_VECTOR(m_Specs.SSE_SNE_Notes.size(), true);
        if (NotesPropertyPresent(m_Specs.SSE_Switch_Rec      )) m_Specs.SSE_Switch_Notes       = BOOL_VECTOR(m_Specs.SSE_Switch_Notes.size(), true);
        if (NotesPropertyPresent(m_Specs.SSE_SysParms_Rec    )) m_Specs.SSE_SysParms_Notes     = BOOL_VECTOR(m_Specs.SSE_SysParms_Notes.size(), true);
        if (NotesPropertyPresent(m_Specs.SSE_Sys_Snapshot_Rec)) m_Specs.SSE_Sys_Snapshot_Notes = BOOL_VECTOR(m_Specs.SSE_Sys_Snapshot_Notes.size(), true);

        // process the logfile definitions file if specified
        m_Files.Enabled = UpdateAllLogfileRecordSpecs();                                                                          // update all logfile record specifications - disable logging upon failure

        if (m_Files.Enabled) {                                                                                                    // still ok?
                                                                                                                            // yes
            ...
            if (m_Config.LogBasePathString != ".") {                                                                               // CWD?
                if (!boost::filesystem::exists(m_Config.LogBasePathString)) {                                                      // no - user-specifed base path already exists?                                                                                                         
                    std::tie(error, errStr, m_Config.LogPathsCreated) = utils::CreateDirectories(m_Config.LogBasePathString);             // no - create directories
                    if (error != ERROR::NONE) {                                                                             // ok?
                        DBG_WARN(ERR_MSG(ERROR::UNABLE_TO_CREATE_DIRECTORY) + " '" + m_Config.LogBasePathString + "': Using CWD"); // no - show warning
                        m_Config.LogBasePathString = boost::filesystem::current_path().string();                                   // use CWD as base path
                    }
                }
            }
            m_Config.LogBasePathString = boost::filesystem::canonical(m_Config.LogBasePathString).string();                               // get fully-qualified path string

            // now create the container folder at p_LogBasePath

            string containerName = p_LogContainerName;                                                                      // container name
            m_Hdf5.ContainerName  = DEFAULT_HDF5_FILE_NAME;                                                                  // HDF5 container name
            string dirName       = containerName;                                                                           // directory name to create

            int version = 0;                                                                                                // container version number if required - start at 1
            while (boost::filesystem::exists(m_Config.LogBasePathString + "/" + dirName)) {                                        // container already exists?
                dirName = containerName + "_" + std::to_string(++version);                                                  // yes - add a version number and generate new container name
            }
            m_Config.LogContainerName = dirName;                                                                                   // record actual container directory name

            try {
                (void)boost::filesystem::create_directory(m_Config.LogBasePathString + "/" + m_Config.LogContainerName, err);             // create container - let boost throw an exception if it fails
                if (err.value() == 0) {                                                                                     // ok?

                    m_Config.LogPathsCreated.push_back(m_Config.LogBasePathString + "/" + m_Config.LogContainerName);                            // yes - add entry to paths created vector

                    if (m_Config.DbgToLogfile) {                                                                                   // write dubug output to a logfile?
                        ...
                        if (id >= 0) {                                                                                      // success
                            m_Config.DbgLogfileId = id;                                                                            // record the file id
                        }
                        else {                                                                                              // failure
                    ...
                    if (m_Config.ErrToLogfile) {                                                                                   // write dubug output to a logfile?
                        string filename = std::get<0>(LOGFILE_DESCRIPTOR.at(LOGFILE::ERROR_LOG));                           // extract filename from descriptor
                        int id = Open(filename, false, true, false);                                                        // open the log file - new file, timestamps, no record labels, space delimited
                        if (id >= 0) {                                                                                      // success
                            m_Config.ErrLogfileId = id;                                                                            // record the file id
                        }
                        else {                                                                                              // failure
        ...
        if (m_Files.Enabled) {                                                                                                    // still ok?
            ...
            if (m_Config.LogfileType == LOGFILETYPE::HDF5) {                                                                       // logging to HDF5 files?
                                                                                                                            // yes
                string fileExt    = "." + LOGFILETYPEFileExt.at(OPTIONS->LogfileType());                                    // file extension for HDF5 files
                string h5Filename = m_Config.LogBasePathString + "/" + m_Config.LogContainerName + "/" + m_Hdf5.ContainerName + fileExt;   // full filename with path, container, and extension ("/" works on Uni*x and Windows)
                m_Hdf5.ContainerId = H5Fcreate(h5Filename.c_str(), H5F_ACC_EXCL, H5P_DEFAULT, H5P_DEFAULT);                  // create HDF5 container file
                if (m_Hdf5.ContainerId < 0) {                                                                                // created ok?                        
                    Squawk("ERROR: Unable to create HDF5 container file with file name " + h5Filename);                     // no - announce error
                    Squawk("Logging disabled");                                                                             // show disabled warning
                    m_Files.Enabled = false;                                                                                      // disable logging
                }
                else {                                                                                                      // HDF5 container file now exists and open
                    m_Files.Enabled = OpenHDF5RunDetailsFile(RUN_DETAILS_FILE_NAME);                                              // open HDF5 run details file inside HDF5 container
                    if (!m_Files.Enabled) Squawk("Logging disabled");                                                             // show disabled warning
                }
            }
            ...
            if (m_Files.Enabled) {                                                                                                // still ok?
                                                                                                                            // yes
                string filename = m_Config.LogBasePathString + "/" + m_Config.LogContainerName + "/" + RUN_DETAILS_FILE_NAME;             // run details (text) filename with container name
                try {
                    m_Files.RunDetailsFile.open(filename, std::ios::out);                                                         // create run details (text) file
                    m_Files.RunDetailsFile.exceptions(std::ofstream::failbit | std::ofstream::badbit);                            // enable exceptions on run details file
                }
            ...
            if (OPTIONS->StoreInputFiles()) {                                                                               // user wants input files stored in output container?
                                                                                                                            // yes
                string dstPath = m_Config.LogBasePathString + "/" + m_Config.LogContainerName + "/";                                      // destination path (output container)
                ...
                if (m_Files.Enabled && !OPTIONS->LogfileDefinitionsFilename().empty()) {                                          // user specified a logfile-definitions file?
                    try {                                                                                                   // yes - copy it
...
void Log::Stop(std::tuple<int, int> p_ObjectStats) {
    ...
    if (m_Files.Enabled) {                                                                                                                    // only need to do most of this if logging is enabled 

        // get some run stats
     
        double cpuSeconds = (clock() - m_Files.ClockStart) / (double) CLOCKS_PER_SEC;                                                         // stop CPU timer and calculate seconds

        m_Files.WallEnd = std::chrono::system_clock::now();                                                                                   // stop wall timer
        std::time_t timeEnd = std::chrono::system_clock::to_time_t(m_Files.WallEnd);                                                          // get end time and date

        std::chrono::duration<double> wallSeconds = m_Files.WallEnd - m_Files.WallStart;                                                            // elapsed seconds
        ...
        std::time_t timeStart = std::chrono::system_clock::to_time_t(m_Files.WallStart);                                                      // convert start time
                ...
                if (ok) {                                                                                                               // have valid property
                    ...
                    switch (static_cast<RUN_DETAILS_COLUMNS>(idx)) {                                                                    // which dataset?
                        case RUN_DETAILS_COLUMNS::COMPAS_VERSION:                                                                       // COMPAS_Version
                            m_Hdf5.RunDetailsFile.dataSets[dSetIdx].buf.push_back(VERSION_STRING);                                      // add write data to buffer
                            break;

                        case RUN_DETAILS_COLUMNS::RUN_START:                                                                            // Run_Start
                            ss << std::ctime(&timeStart);                                                                               // get start time string
                            m_Hdf5.RunDetailsFile.dataSets[dSetIdx].buf.push_back(ss.str());                                            // add write data to buffer
                        ...
                        case RUN_DETAILS_COLUMNS::OBJECTS_REQUESTED:                                                                    // Objects_Requested
                            m_Hdf5.RunDetailsFile.dataSets[dSetIdx].buf.push_back(objectsRequested);                                    // add write data to buffer
                        ...
                        case RUN_DETAILS_COLUMNS::OBJECTS_CREATED:                                                                      // Objects_Created
                            m_Hdf5.RunDetailsFile.dataSets[dSetIdx].buf.push_back(objectsCreated);                                      // add write data to buffer
                            break;
                            
                        case RUN_DETAILS_COLUMNS::CLOCK_TIME:                                                                           // Clock_Time (CPU seconds)
                            m_Hdf5.RunDetailsFile.dataSets[dSetIdx].buf.push_back(cpuSeconds);                                          // add write data to buffer
                            break;
                          
                        case RUN_DETAILS_COLUMNS::WALL_TIME:                                                                            // Wall_Time (elapsed time: hhhh:mm:ss)
                            m_Hdf5.RunDetailsFile.dataSets[dSetIdx].buf.push_back(wallTime);                                            // add write data to buffer
                            break;
                            
                        case RUN_DETAILS_COLUMNS::ACTUAL_RANDOM_SEED:                                                                   // Actual_Random_Seed
                            m_Hdf5.RunDetailsFile.dataSets[dSetIdx].buf.push_back(actualRandomSeed);                                    // add write data to buffer
                            break;
                    ...
                    if (ok) {
                        if (!WriteHDF5_(m_Hdf5.RunDetailsFile, RUN_DETAILS_FILE_NAME, dSetIdx)) {                                       // write to file ok?
                            ...
                            m_Hdf5.RunDetailsFile.dataSets[dSetIdx].buf.push_back(std::string("CALCULATED"));                           // add write data to buffer
                            if (!WriteHDF5_(m_Hdf5.RunDetailsFile, RUN_DETAILS_FILE_NAME, dSetIdx)) {                                   // write to file ok?
                                Squawk("ERROR: Error writing to HDF5 dataset with name " + h5DatasetName);                              // no - announce error
                                ok = false;                                                                                             // fail
                            }
            ...
            if (ok) {
                ...
                try {
                    for (std::size_t idx = 0; idx < m_Config.OptionDetails.size(); idx++) {                                                    // for each program option
                        ...
                        h5DatasetName = m_Config.OptionDetails[idx].optionStr;                                                                 // dataset name
                        string strValue = m_Config.OptionDetails[idx].valueStr;                                                                // value formatted as string

                        dSetIdx++;                                                                                                      // incremement run details dataset
                        TYPENAME compasType = m_Config.OptionDetails[idx].dataType;                                                            // COMPAS datatype
                        switch (compasType) {
                            case TYPENAME::INT         : m_Hdf5.RunDetailsFile.dataSets[dSetIdx].buf.push_back(std::stoi(strValue));   break;
                            case TYPENAME::LONGINT     : m_Hdf5.RunDetailsFile.dataSets[dSetIdx].buf.push_back(std::stol(strValue));   break;
                            case TYPENAME::LONGLONGINT : m_Hdf5.RunDetailsFile.dataSets[dSetIdx].buf.push_back(std::stoll(strValue));  break;
                            case TYPENAME::ULONGINT    : m_Hdf5.RunDetailsFile.dataSets[dSetIdx].buf.push_back(std::stoul(strValue));  break;                   
                            case TYPENAME::ULONGLONGINT: m_Hdf5.RunDetailsFile.dataSets[dSetIdx].buf.push_back(std::stoull(strValue)); break;                   
                            case TYPENAME::FLOAT       : m_Hdf5.RunDetailsFile.dataSets[dSetIdx].buf.push_back(std::stof(strValue));   break;
                            case TYPENAME::DOUBLE      : m_Hdf5.RunDetailsFile.dataSets[dSetIdx].buf.push_back(std::stod(strValue));   break;
                            case TYPENAME::LONGDOUBLE  : m_Hdf5.RunDetailsFile.dataSets[dSetIdx].buf.push_back(std::stold(strValue));  break;
                            case TYPENAME::STRING      : m_Hdf5.RunDetailsFile.dataSets[dSetIdx].buf.push_back(strValue);              break;
                            case TYPENAME::BOOL        :
                                // boolean string value here is "TRUE or "FALSE"
                                // convert to 1 or 0 if necessary
                                if (OPTIONS->PrintBoolAsString())
                                    m_Hdf5.RunDetailsFile.dataSets[dSetIdx].buf.push_back(strValue);
                                else
                                    m_Hdf5.RunDetailsFile.dataSets[dSetIdx].buf.push_back(strValue == "TRUE" ? true : false);
                                break;
                        ...
                        if (ok) {
                            if (!WriteHDF5_(m_Hdf5.RunDetailsFile, RUN_DETAILS_FILE_NAME, dSetIdx)) {                                   // write to file ok?
                                ...
                                m_Hdf5.RunDetailsFile.dataSets[dSetIdx].buf.push_back(m_Config.OptionDetails[idx].sourceStr);                  // add write data to buffer
                                if (!WriteHDF5_(m_Hdf5.RunDetailsFile, RUN_DETAILS_FILE_NAME, dSetIdx)) {                               // write to file ok?
                                    Squawk("ERROR: Error writing to HDF5 dataset with name " + h5DatasetName);                          // no - announce error
                                    ok = false;                                                                                         // fail
                                }
        ...
        string filename = m_Config.LogBasePathString + "/" + m_Config.LogContainerName + "/" + RUN_DETAILS_FILE_NAME;                                 // run details filename with container name
        try {  
            m_Files.RunDetailsFile << utils::SplashScreen(false) << std::endl;                                                                // write splash string with version number to file

            // record start time and whether evolving single stars or binaries   
            if (OPTIONS->EvolutionMode() == EVOLUTION_MODE::SSE_HURLEY)
                m_Files.RunDetailsFile << "Start generating stars at " << std::ctime(&timeStart) << std::endl;
            else
                m_Files.RunDetailsFile << "Start generating binaries at " << std::ctime(&timeStart) << std::endl;

            // record end time and whether evolving single stars or binaries   
            if (OPTIONS->EvolutionMode() == EVOLUTION_MODE::SSE_HURLEY) {
                m_Files.RunDetailsFile << "Generated " << std::to_string(objectsCreated) << " of " << (objectsRequested < 0 ? "<INCOMPLETE GRID>" : std::to_string(objectsRequested)) << " stars requested" << std::endl;
                m_Files.RunDetailsFile << "\nEnd generating stars at " << std::ctime(&timeEnd) << std::endl;
            }
            else {
                m_Files.RunDetailsFile << "Generated " << std::to_string(objectsCreated) << " of " << (objectsRequested < 0 ? "<INCOMPLETE GRID>" : std::to_string(objectsRequested)) << " binaries requested" << std::endl;
                m_Files.RunDetailsFile << "\nEnd generating binaries at " << std::ctime(&timeEnd) << std::endl; 
            }

            m_Files.RunDetailsFile << "Clock time = " << cpuSeconds << " CPU seconds" << std::endl;                                           // record cpu seconds

            m_Files.RunDetailsFile << "Wall time  = " << wallTime << " (hhhh:mm:ss)" << std::endl;                                            // wall time 
            ...
            for (std::size_t idx = 0; idx < m_Config.OptionDetails.size(); idx++) {                                                            // and add them to the run details file

                if (utils::Equals(m_Config.OptionDetails[idx].sourceStr, "CALCULATED")) continue;                                              // CALCULATED later

                m_Files.RunDetailsFile << m_Config.OptionDetails[idx].optionStr << " = ";                                                            // option name

                if (m_Config.OptionDetails[idx].valueStr == "")                                                                                // empty option?
                    m_Files.RunDetailsFile << "<EMPTY_OPTION>\n";                                                                             // yes - say so
                else                                                                                                                    // no - add option details
                    m_Files.RunDetailsFile << m_Config.OptionDetails[idx].valueStr + ", "                                                            // value
                                     << m_Config.OptionDetails[idx].sourceStr + ", "                                                           // defaulted
                                     << m_Config.OptionDetails[idx].typeStr << "\n";                                                           // datatype
            }
            ...
            for (std::size_t idx = 0; idx < m_Config.OptionDetails.size(); idx++) {                                                            // and add them to the run details file
                ...
                if (!utils::Equals(m_Config.OptionDetails[idx].sourceStr, "CALCULATED")) continue;                                             // only CALCULATED here
            ...
            m_Files.RunDetailsFile << "actual-output-path = " << m_Config.LogBasePathString << ", CALCULATED, STRING"  << std::endl;                 // actual output path
            m_Files.RunDetailsFile << "actual-output-container = " << m_Config.LogContainerName << ", CALCULATED, STRING"  << std::endl;             // actual output container
            m_Files.RunDetailsFile << "actual-random-seed = " << actualRandomSeed  << ", CALCULATED, UNSIGNED_LONG" << std::endl;             // actual random seed

            // done writing - flush and close the file
            try {
                m_Files.RunDetailsFile.flush();
                m_Files.RunDetailsFile.close();
            }
        ...
        for(unsigned int index = 0; index < m_Files.Logfiles.size(); index++) {                                                               // check for open logfiles (even if not active)
            if (IsActiveId(index)) {                                                                                                    // logfile active?
                if (m_Files.Logfiles[index].file.is_open()) {                                                                                 // open file?
                    try {                                                                                                               // yes
                        m_Files.Logfiles[index].file.flush();                                                                                 // flush output and
                        m_Files.Logfiles[index].file.close();                                                                                 // close it
                    }
                    catch (const std::ofstream::failure &e) {                                                                           // problem...
                        Squawk("ERROR: Unable to close log file with file name " + m_Files.Logfiles[index].name);                             // announce error
                        Squawk(e.what());                                                                                               // plus details
                    }
                }
            }
    ...
    (void) utils::RemoveDirectories(m_Config.LogPathsCreated);

    m_Files.Logfiles.clear();                                                                                                                 // clear all entries
    m_Files.Enabled = false;                                                                                                                  // set not enabled
}
 ...
 *
 * New log file is created at path m_Config.LogBasePathString
 *
 ...
 * @return                                      Logfile id (integer index into m_Files.Logfiles vector).  A value of -1 indicates log file not opened successfully.
 */
int Log::Open(const string p_LogFileName, const bool p_Append, const bool p_Timestamp, const bool p_Label, const LOGFILE p_StandardLogfile) {
    ...
    if (m_Files.Enabled) {                                                                                                // logging enabled?   
        ...
        string basename = m_Config.LogBasePathString + "/" + m_Config.LogContainerName + "/" + m_Config.LogNamePrefix + p_LogFileName;   // base filename with path and container ("/" works on Uni*x and Windows)
        ...
        if (m_Config.LogfileType == LOGFILETYPE::HDF5) {                                                                   // HDF5 file?
                                                                                                                    // yes
            ...
            if (p_StandardLogfile == LOGFILE::SSE_DETAILED_OUTPUT || p_StandardLogfile == LOGFILE::BSE_DETAILED_OUTPUT) { // detailed output file?
                h5FileId  = m_Hdf5.DetailedId;                                                                       // yes - use detailed file id
                h5GroupId = h5FileId;                                                                               // no group for detailed file - just use the file id
            }
            else {                                                                                                  // no, not detailed output file
                h5FileId    = m_Hdf5.ContainerId;                                                                    // container file id
                h5GroupName = m_Config.LogNamePrefix + p_LogFileName;                                                      // HDF5 file group name

                if (m_Hdf5.ContainerId >= 0) {                                                                       // yes - have HDF5 container file?
                    h5GroupId = H5Gopen(m_Hdf5.ContainerId, h5GroupName.c_str(), H5P_DEFAULT);                       // yes - open the group
                    if (h5GroupId < 0) {                                                                            // group open (and therefore already exists)?
                        h5GroupId = H5Gcreate(m_Hdf5.ContainerId, h5GroupName.c_str(), 0, H5P_DEFAULT, H5P_DEFAULT); // no - create the group
                        if (h5GroupId < 0) {                                                                        // group created ok?
            ...
            if (ok) {                                                                                               // still ok?
                ...
                // record attributes
                // find an empty slot in m_Files.Logfiles vector if there is one
                // this way is a bit slower for opening logfiles, but faster for writing to them
                id = -1;  
                for(unsigned int index = 0; index < m_Files.Logfiles.size(); index++) {
                    if (!m_Files.Logfiles[index].active) {                                                                // empty slot?
                        id = index;                                                                                 // yes - use it
                ...
                if (id < 0) {                                                                                       // have empty slot?
                    ...
                    id = m_Files.Logfiles.size();                                                                         // set new id
                    m_Files.Logfiles.push_back(std::move(attr));                                                          // append to vector
                }

                m_Files.Logfiles[id].active           = true;                                                             // this entry now active
                m_Files.Logfiles[id].logfiletype      = p_StandardLogfile;                                                // standard logfile type
                m_Files.Logfiles[id].filetype         = m_Config.LogfileType;                                                    // set filetype for this log file
                m_Files.Logfiles[id].name             = h5GroupName;                                                      // log file name (HDF5 group name)
                m_Files.Logfiles[id].timestamp        = p_Timestamp;                                                      // set timestamp flag for this log file
                m_Files.Logfiles[id].label            = p_Label;                                                          // set label flag for this log file
                m_Files.Logfiles[id].h5File.fileId    = h5FileId;                                                         // HDF5 file id
                m_Files.Logfiles[id].h5File.groupId   = h5GroupId;                                                        // HDF5 group id
                m_Files.Logfiles[id].h5File.dataSets  = {};                                                               // HDF5 data sets
            }
        }
        else {                                                                                                      // no - not HDF5
            // record attributes
            ...
            if (id < 0) {                                                                                           // have empty slot?
                id = m_Files.Logfiles.size();                                                                             // no - set new id
            ...
            try {
                m_Files.Logfiles[id].file.open(filename, std::ios::out | std::ios::app);                                  // create fs log file
                m_Files.Logfiles[id].file.exceptions(std::ofstream::failbit | std::ofstream::badbit);                     // enable exceptions on log file

                m_Files.Logfiles[id].active         = true;                                                               // this entry now active
                m_Files.Logfiles[id].logfiletype    = p_StandardLogfile;                                                  // standard logfile type
                m_Files.Logfiles[id].filetype       = m_Config.LogfileType;                                                      // set filetype for this log file
                m_Files.Logfiles[id].name           = filename;                                                           // log file name
                m_Files.Logfiles[id].timestamp      = p_Timestamp;                                                        // set timestamp flag for this log file
                m_Files.Logfiles[id].label          = p_Label;                                                            // set label flag for this log file
                m_Files.Logfiles[id].h5File.fileId  = -1;                                                                 // not HDF5 file
                m_Files.Logfiles[id].h5File.groupId = -1;                                                                 // not HDF5 file
            }
            catch (const std::ofstream::failure &e) {                                                               // fs problem...
                Squawk("ERROR: Unable to create fs log file with file name " + m_Files.Logfiles[id].name);                // announce error
                Squawk(e.what());                                                                                   // plus details

                // it's possible this m_Files.Logfiles entry was just appended - it can be used next time
...
bool Log::Close(const int p_LogfileId) {
    ...
    if (m_Files.Enabled) {                                                                                            // logging enabled?
        ...
        if (standardFile) {                                                                                     // file is an open standard file
            ...
            iter = m_Files.OpenStandardLogFileIds.find(logfile);                                                      // get the details
            if (iter != m_Files.OpenStandardLogFileIds.end()) {                                                       // found
        ...
        if (result && standardFile) {                                                                           // if it was a standard file...
            m_Files.OpenStandardLogFileIds.erase(logfile);                                                            // ...remove it from the map
        }
    }
...
bool Log::Close_(const int p_LogfileId) {
    ...
    if (m_Files.Enabled && IsActiveId(p_LogfileId)) {                                                                     // logging enabled and logfile active?
        if (m_Files.Logfiles[p_LogfileId].filetype == LOGFILETYPE::HDF5) {                                                // yes - HDF5 logfile?
            Flush_(p_LogfileId);                                                                                    // first, flush any unwritten data

            // close all open datasets
            for (auto &dataSet : m_Files.Logfiles[p_LogfileId].h5File.dataSets) {                                         // for each dataset
                if (dataSet.dataSetId >= 0) {                                                                       // dataset open?
                                                                                                                    // yes - close it
                    if (H5Dclose(dataSet.dataSetId) < 0) {                                                          // closed ok?
                        Squawk("ERROR: Unable to close HDF5 dataset for log file " + m_Files.Logfiles[p_LogfileId].name); // no - announce error
                        result = false;                                                                             // fail
                    }
                }
            }
            ...
            if (m_Files.Logfiles[p_LogfileId].logfiletype != LOGFILE::SSE_DETAILED_OUTPUT && 
                m_Files.Logfiles[p_LogfileId].logfiletype != LOGFILE::BSE_DETAILED_OUTPUT) {                              // detailed output file?

                if (m_Files.Logfiles[p_LogfileId].h5File.groupId >= 0) {                                                  // no - HDF5 group open?
                    if (H5Gclose(m_Files.Logfiles[p_LogfileId].h5File.groupId) < 0) {                                     // yes - closed ok?
                        Squawk("ERROR: Unable to close HDF5 group for log file " + m_Files.Logfiles[p_LogfileId].name);   // no - announce error
                        result = false;                                                                             // fail
                    }
                }
            }
            else {                                                                                                  // yes - detailed output file
                if (H5Fclose(m_Files.Logfiles[p_LogfileId].h5File.fileId) < 0) {                                          // HDF5 file closed ok?
                    Squawk("ERROR: Unable to close HDF5 file " + m_Files.Logfiles[p_LogfileId].name);                     // no - announce error
                    result = false;                                                                                 // fail
                }
                m_Hdf5.DetailedId = -1;                                                                              // (should have) no open detailed output file
            }
        }
        else {                                                                                                      // no, FS logfile
            if (m_Files.Logfiles[p_LogfileId].file.is_open()) {                                                           // log file open?
                try {                                                                                               // yes
                    m_Files.Logfiles[p_LogfileId].file.flush();                                                           // flush output and
                    m_Files.Logfiles[p_LogfileId].file.close();                                                           // close it
                }
                catch (const std::ofstream::failure &e) {                                                           // problem...
                    Squawk("ERROR: Unable to close log file with file name " + m_Files.Logfiles[p_LogfileId].name);       // announce error
                    Squawk(e.what());                                                                               // plus details
                    result = false;                                                                                 // fail
                }
            }
...
void Log::Say(const string p_SayClass, const int p_SayLevel, const string p_SayStr) {
    if (m_Files.Enabled) {                                                                                                // logging service enabled?
        if (DoIt(p_SayClass, p_SayLevel, m_Config.LogClasses, m_Config.LogLevel)) {                                               // logging this class and level?
...
bool Log::Write(const int p_LogfileId, const string p_LogClass, const int p_LogLevel, const string p_LogStr) {
    ...
    if (m_Files.Enabled && IsActiveId(p_LogfileId)) {                                                                     // logging service enabled and specified log file active?
        if (DoIt(p_LogClass, p_LogLevel, m_Config.LogClasses, m_Config.LogLevel)) {                                               // yes - logging this class and level?
...
bool Log::Write_(const int p_LogfileId, const string p_LogStr) {
    ...
    if (m_Files.Enabled && IsActiveId(p_LogfileId)) {                                                                     // logging service enabled and specified log file active?
        try {
            m_Files.Logfiles[p_LogfileId].file << p_LogStr << std::endl;                                                  // write string to log file
            m_Files.Logfiles[p_LogfileId].file.flush();                                                                   // flush data to log file

            result = true;                                                                                          // set result
        }
        catch (const std::ofstream::failure &e) {                                                                   // problem...
            Squawk("ERROR: Unable to write to log file with file name " + m_Files.Logfiles[p_LogfileId].name);            // announce error
            Squawk(e.what());                                                                                       // plus details
...
bool Log::WriteHDF5_(h5AttrT& p_H5file, const string p_H5filename, const size_t p_DataSetIdx) {
    ...
    if (ok >= 0) {                                                                                                          // good to write?
        ...
        // for each datatype:
        //   - create a buffer of correct C++ type
        ...
        if (dType == H5T_NATIVE_UCHAR) {
            ...
            ok = H5Dwrite(dSet, dType, h5Dspace, h5FSpace, H5P_DEFAULT, buf.data());
        }
        else if (dType == H5T_NATIVE_SHORT) {
            std::vector<short int> buf(bufSize);
            ...
            std::vector<unsigned int> buf(bufSize);
            ...
            std::vector<unsigned long int> buf(bufSize);
            ...
            // Both TYPENAME::DOUBLE and TYPENAME::CDOUBLE map to H5T_NATIVE_DOUBLE
            const TYPENAME dt = p_H5file.dataSets[p_DataSetIdx].dataType;
            for (size_t i = 0; i < bufSize; i++) {
                buf[i] = (dt == TYPENAME::CDOUBLE)
                             ? boost::get<CDOUBLE>(p_H5file.dataSets[p_DataSetIdx].buf[i]).Value()
                             : boost::get<double>(p_H5file.dataSets[p_DataSetIdx].buf[i]);
            }
            COMPAS_VARIABLE_VECTOR().swap(p_H5file.dataSets[p_DataSetIdx].buf);
            ok = H5Dwrite(dSet, dType, h5Dspace, h5FSpace, H5P_DEFAULT, buf.data());
        }
        else if (dType == H5T_NATIVE_LDOUBLE) {
            std::vector<long double> buf(bufSize);
            ...
            std::vector<std::string> buf(bufSize);
            ...
            if (fixedLength) {                                                                                              // fixed-length string?
                std::vector<char> cBuf(bufSize * (elemLen + 1));                                                            // H5Dwrite() needs contiguous memory
                for (size_t i = 0; i < bufSize; i++) {                                                                      // for each entry in the buffer
                    char*  dst = cBuf.data() + i * (elemLen + 1);                                                           // point at next elem
                    size_t n   = std::min(buf[i].length(), elemLen);                                                        // clamped to elemLen
                    std::memcpy(dst, buf[i].data(), n);                                                                     // copy elem - truncated if too long
                    dst[elemLen] = '\0';                                                                                    // null-terminate
                }

                ok = H5Dwrite(dSet, dType, h5Dspace, h5FSpace, H5P_DEFAULT, (const void *)cBuf.data());                     // write the data
            }
            else {                                                                                                          // no - variable length
                std::vector<const char*> cBuf(bufSize);                                                                     // char array for H5Dwrite()
                for (size_t i = 0; i < bufSize; i++) cBuf[i] = buf[i].c_str();                                              // H5Dwrite() needs contiguous memory only for each element
...
bool Log::Write_(const int p_LogfileId, const std::vector<COMPAS_VARIABLE> p_LogRecordValues, const bool p_Flush) {
    ...
    if (m_Files.Logfiles[p_LogfileId].filetype != LOGFILETYPE::HDF5) return ok;                                                   // shouldn't be here if not HDF5 logfile

    if (m_Files.Enabled && IsActiveId(p_LogfileId)) {                                                                             // logging service enabled and specified log file active?
        if ((ok = m_Files.Logfiles[p_LogfileId].h5File.fileId) < 0) {                                                             // HDF5 file open?
            Squawk("ERROR: Unable to write to HDF5 log file " + m_Files.Logfiles[p_LogfileId].name);                              // no - announce error
        }
        else {
            if (m_Files.Logfiles[p_LogfileId].h5File.groupId < 0) {                                                               // HDF5 group open?
                Squawk("ERROR: Unable to write to HDF5 group for log file " + m_Files.Logfiles[p_LogfileId].name);                // no - announce error
            }
            else {

                for (size_t idx = 0; idx < m_Files.Logfiles[p_LogfileId].h5File.dataSets.size(); idx++) {                         // for each dataset

                    hid_t dSet  = m_Files.Logfiles[p_LogfileId].h5File.dataSets[idx].dataSetId;                                   // dataset id

                    if (dSet >= 0) {                                                                                        // dataset open?
                                                                                                                            // yes
                        if (!p_Flush) {                                                                                     // flush only?
                            m_Files.Logfiles[p_LogfileId].h5File.dataSets[idx].buf.push_back(p_LogRecordValues[idx]);             // no - add write data to buffer
                        }

                        if ((m_Files.Logfiles[p_LogfileId].h5File.dataSets[idx].buf.size() >= m_Files.Logfiles[p_LogfileId].h5File.IOBufSize) || p_Flush) { // need to write?
                            ok = WriteHDF5_(m_Files.Logfiles[p_LogfileId].h5File, m_Files.Logfiles[p_LogfileId].name, idx);             // do the write 
                        }
                    }
                }
            }
...
bool Log::Put_(const int p_LogfileId, const string p_LogStr, const string p_Label) {
    ...
    if (m_Files.Enabled && IsActiveId(p_LogfileId)) {                                                                     // logging service enabled and specified log file active?
        ...
        switch (m_Files.Logfiles[p_LogfileId].filetype) {
        ...
        string timestamp;
        if (m_Files.Logfiles[p_LogfileId].timestamp) {                                                              // timestamp enabled?
        ...
        string logStr = "";                                                                                         // initialise the output string
               logStr += m_Files.Logfiles[p_LogfileId].timestamp ? timestamp + delimiter : "";                      // add timestamp if required
               logStr += m_Files.Logfiles[p_LogfileId].label && p_Label.length() > 0 ? p_Label + delimiter : "";    // add record label if required (may be blank)
...
bool Log::Debug(const string p_DbgClass, const int p_DbgLevel, const string p_DbgStr) {
    ...
    if (m_Files.Enabled) {                                                                                                // logging service enabled?
        if (DoIt(p_DbgClass, p_DbgLevel, m_Config.DbgClasses, m_Config.DbgLevel)) {                                               // debugging this class and level?
...
bool Log::Debug_(const string p_DbgStr) {
    ...
    if (m_Files.Enabled) {                                                                                                // logging service enabled?
        ...
        if (m_Config.DbgToLogfile) {                                                                                       // also to log file?
            result = Put_(m_Config.DbgLogfileId, p_DbgStr);                                                                // log it
        }
    }
...
bool Log::Error(const string p_ErrStr) {
    ...
    if (m_Config.ErrToLogfile && m_Files.Enabled) {                                                                              // but do need it enabled to write error to logfile
        result = Put_(m_Config.ErrLogfileId, p_ErrStr);                                                                    // log it
    }
...
/*
 * Find a value in the m_Files.OpenStandardLogFileIds map and return the key if found, otherwise default value
 *
 * This function looks for the file id value in the m_Files.OpenStandardLogFileIds map, and if the id is found
...
std::tuple<bool, LOGFILE> Log::GetStandardLogfileKey(const int p_FileId) {
    for (auto& it: m_Files.OpenStandardLogFileIds)
    ...
    try {
        ...
        switch (p_Logfile) {                                                                                                        // which logfile?
            ...
            case LOGFILE::BSE_COMMON_ENVELOPES:                                                                                     // BSE_COMMON_ENVELOPES
                recordProperties = m_Specs.BSE_CEE_Rec;                                                                                   // record properties
                annotations      = m_Specs.BSE_CEE_Notes;                                                                                 // logfile annotations
                break;

            case LOGFILE::BSE_DETAILED_OUTPUT:                                                                                      // BSE_DETAILED_OUTPUT
                recordProperties = m_Specs.BSE_Detailed_Rec;                                                                              // record properties
                annotations      = m_Specs.BSE_Detailed_Notes;                                                                            // logfile annotations
                break;

            case LOGFILE::BSE_DOUBLE_COMPACT_OBJECTS:                                                                               // BSE_DOUBLE_COMPACT_OBJECTS
                recordProperties = m_Specs.BSE_DCO_Rec;                                                                                   // record properties
                annotations      = m_Specs.BSE_DCO_Notes;                                                                                 // logfile annotations
                break;

            case LOGFILE::BSE_PULSAR_EVOLUTION:                                                                                     // BSE_PULSAR_EVOLUTION
                recordProperties = m_Specs.BSE_Pulsars_Rec;                                                                               // record properties
                annotations      = m_Specs.BSE_Pulsars_Notes;                                                                             // logfile annotations
                break;

            case LOGFILE::BSE_RLOF_PARAMETERS:                                                                                      // BSE_RLOF_PARAMETERS
                recordProperties = m_Specs.BSE_RLOF_Rec;                                                                                  // record properties
                annotations      = m_Specs.BSE_RLOF_Notes;                                                                                // logfile annotations
                break;

            case LOGFILE::BSE_SUPERNOVAE:                                                                                           // BSE_SUPERNOVAE
                recordProperties = m_Specs.BSE_SNE_Rec;                                                                                   // record properties
                annotations      = m_Specs.BSE_SNE_Notes;                                                                                 // logfile annotations
                break;

            case LOGFILE::BSE_SWITCH_LOG:                                                                                           // BSE_SWITCH_LOG
                recordProperties = m_Specs.BSE_Switch_Rec;                                                                                // record properties
                annotations      = m_Specs.BSE_Switch_Notes;                                                                              // logfile annotations
                break;

            case LOGFILE::BSE_SYSTEM_SNAPSHOT_LOG:                                                                                  // BSE_SYSTEM_SNAPSHOT_LOG
                recordProperties = m_Specs.BSE_Sys_Snapshot_Rec;                                                                          // record properties
                annotations      = m_Specs.BSE_Sys_Snapshot_Notes;                                                                        // logfile annotations
                break;

            case LOGFILE::BSE_SYSTEM_PARAMETERS:                                                                                    // BSE_SYSTEM_PARAMETERS
                recordProperties = m_Specs.BSE_SysParms_Rec;                                                                              // record properties
                annotations      = m_Specs.BSE_SysParms_Notes;                                                                            // logfile annotations
            ...
            case LOGFILE::SSE_DETAILED_OUTPUT:                                                                                      // SSE_DETAILED_OUTPUT
                recordProperties = m_Specs.SSE_Detailed_Rec;                                                                              // record properties
                annotations      = m_Specs.SSE_Detailed_Notes;                                                                            // logfile annotations
                break;

            case LOGFILE::SSE_SUPERNOVAE:                                                                                           // SSE_SUPERNOVAE
                recordProperties = m_Specs.SSE_SNE_Rec;                                                                                   // record properties
                annotations      = m_Specs.SSE_SNE_Notes;                                                                                 // logfile annotations
                break;

            case LOGFILE::SSE_SWITCH_LOG:                                                                                           // SSE_SWITCH_LOG
                recordProperties = m_Specs.SSE_Switch_Rec;                                                                                // record properties
                annotations      = m_Specs.SSE_Switch_Notes;                                                                              // logfile annotations
                break;

            case LOGFILE::SSE_PULSAR_EVOLUTION:                                                                                     // SSE_PULSAR_EVOLUTION
                recordProperties = m_Specs.SSE_Pulsars_Rec;                                                                               // record properties
                annotations      = m_Specs.SSE_Pulsars_Notes;                                                                             // logfile annotations
                break;

            case LOGFILE::SSE_SYSTEM_SNAPSHOT_LOG:                                                                                  // SSE_SYSTEM_SNAPSHOT_LOG
                recordProperties = m_Specs.SSE_Sys_Snapshot_Rec;                                                                          // record properties
                annotations      = m_Specs.SSE_Sys_Snapshot_Notes;                                                                        // logfile annotations
                break;

            case LOGFILE::SSE_SYSTEM_PARAMETERS:                                                                                    // SSE_SYSTEM_PARAMETERS
                recordProperties = m_Specs.SSE_SysParms_Rec;                                                                              // record properties
                annotations      = m_Specs.SSE_SysParms_Notes;                                                                            // logfile annotations
...
hid_t Log::GetHDF5DataType(const TYPENAME p_COMPASdatatype, const int p_FieldWidth, const STRING_QUALIFIER p_StringQualifier) {
    ...
    switch (p_COMPASdatatype) {                                                                                     // which COMPAS datatype?
        ...
        case TYPENAME::CDOUBLE          : h5DataType = H5T_NATIVE_DOUBLE; break;
 ...
 * is not already open.
 *
 * The function first checks map of currently open standard logfiles (m_Files.OpenStandardLogFileIds),
 * and if the logfile is found just returns the details recorded there.
 *
 * If the logfile is not found in m_Files.OpenStandardLogFileIds, the logfile details are assembled
...
LogfileDetailsT Log::StandardLogFileDetails(const LOGFILE p_Logfile, const string p_FileSuffix) {
    ...
    logfile = m_Files.OpenStandardLogFileIds.find(p_Logfile);                                                                                         // look for open logfile
    if (logfile == m_Files.OpenStandardLogFileIds.end()) {                                                                                            // doesn't exist
        try {                                                                                                                                   // get record properties for this file
            switch (p_Logfile) {                                                                                                                // which logfile?
                ...
                case LOGFILE::BSE_COMMON_ENVELOPES:                                                                                             // BSE_COMMON_ENVELOPES
                    ...
                    fileDetails.recordProperties = m_Specs.BSE_CEE_Rec;
                    fileDetails.annotations      = m_Specs.BSE_CEE_Notes;
                    break;
                ...
                case LOGFILE::BSE_DOUBLE_COMPACT_OBJECTS:                                                                                       // BSE_DOUBLE_COMPACT_OBJECTS
                    ...
                    fileDetails.recordProperties = m_Specs.BSE_DCO_Rec;
                    fileDetails.annotations      = m_Specs.BSE_DCO_Notes;
                    break;
               
                case LOGFILE::BSE_PULSAR_EVOLUTION:                                                                                             // BSE_PULSAR_EVOLUTION
                    fileDetails.filename         = OPTIONS->LogfilePulsarEvolution();
                    fileDetails.recordTypes      = OPTIONS->LogfilePulsarEvolutionRecordTypes();
                    fileDetails.recordProperties = m_Specs.BSE_Pulsars_Rec;
                    fileDetails.annotations      = m_Specs.BSE_Pulsars_Notes;
                    break;

                case LOGFILE::BSE_RLOF_PARAMETERS:                                                                                              // BSE_RLOF_PARAMETERS
                    fileDetails.filename         = OPTIONS->LogfileRLOFParameters();
                    fileDetails.recordTypes      = OPTIONS->LogfileRLOFParametersRecordTypes();
                    fileDetails.recordProperties = m_Specs.BSE_RLOF_Rec;
                    fileDetails.annotations      = m_Specs.BSE_RLOF_Notes;
                    break;

                case LOGFILE::BSE_SUPERNOVAE:                                                                                                   // BSE_SUPERNOVAE
                    fileDetails.filename         = OPTIONS->LogfileSupernovae();
                    fileDetails.recordTypes      = OPTIONS->LogfileSupernovaeRecordTypes();
                    fileDetails.recordProperties = m_Specs.BSE_SNE_Rec;
                    fileDetails.annotations      = m_Specs.BSE_SNE_Notes;
                    break;

                case LOGFILE::BSE_SWITCH_LOG:                                                                                                   // BSE_SWITCH_LOG
                    fileDetails.filename         = OPTIONS->LogfileSwitchLog();
                    fileDetails.recordTypes      = -1;
                    fileDetails.recordProperties = m_Specs.BSE_Switch_Rec;
                    fileDetails.annotations      = m_Specs.BSE_Switch_Notes;
                    break;

                case LOGFILE::BSE_SYSTEM_SNAPSHOT_LOG:                                                                                          // BSE_SYSTEM_SNAPSHOT_LOG
                    fileDetails.filename         = OPTIONS->LogfileSystemSnapshotLog();
                    fileDetails.recordTypes      = OPTIONS->LogfileSystemSnapshotLogRecordTypes();
                    fileDetails.recordProperties = m_Specs.BSE_Sys_Snapshot_Rec;
                    fileDetails.annotations      = m_Specs.BSE_Sys_Snapshot_Notes;
                    break;

                case LOGFILE::BSE_SYSTEM_PARAMETERS:                                                                                            // BSE_SYSTEM_PARAMETERS
                    fileDetails.filename         = OPTIONS->LogfileSystemParameters();
                    fileDetails.recordTypes      = OPTIONS->LogfileSystemParametersRecordTypes();
                    fileDetails.recordProperties = m_Specs.BSE_SysParms_Rec;
                    fileDetails.annotations      = m_Specs.BSE_SysParms_Notes;
                ...
                case LOGFILE::SSE_SUPERNOVAE:                                                                                                   // SSE_SUPERNOVAE
                    fileDetails.filename         = OPTIONS->LogfileSupernovae();
                    fileDetails.recordTypes      = OPTIONS->LogfileSupernovaeRecordTypes();
                    fileDetails.recordProperties = m_Specs.SSE_SNE_Rec;
                    fileDetails.annotations      = m_Specs.SSE_SNE_Notes;
                    break;

                case LOGFILE::SSE_SWITCH_LOG:                                                                                                   // SSE_SWITCH_LOG
                    fileDetails.filename         = OPTIONS->LogfileSwitchLog();
                    fileDetails.recordTypes      = -1;
                    fileDetails.recordProperties = m_Specs.SSE_Switch_Rec;
                    fileDetails.annotations      = m_Specs.SSE_Switch_Notes;
                    break;

                case LOGFILE::SSE_PULSAR_EVOLUTION:                                                                                             // SSE_PULSAR_EVOLUTION
                    fileDetails.filename         = OPTIONS->LogfilePulsarEvolution();
                    fileDetails.recordTypes      = OPTIONS->LogfilePulsarEvolutionRecordTypes();
                    fileDetails.recordProperties = m_Specs.SSE_Pulsars_Rec;
                    fileDetails.annotations      = m_Specs.SSE_Pulsars_Notes;
                    break;

                case LOGFILE::SSE_SYSTEM_SNAPSHOT_LOG:                                                                                          // SSE_SYSTEM_SNAPSHOT_LOG
                    fileDetails.filename         = OPTIONS->LogfileSystemSnapshotLog();
                    fileDetails.recordTypes      = OPTIONS->LogfileSystemSnapshotLogRecordTypes();
                    fileDetails.recordProperties = m_Specs.SSE_Sys_Snapshot_Rec;
                    fileDetails.annotations      = m_Specs.SSE_Sys_Snapshot_Notes;
                    break;

                case LOGFILE::SSE_SYSTEM_PARAMETERS:                                                                                            // SSE_SYSTEM_PARAMETERS
                    fileDetails.filename         = OPTIONS->LogfileSystemParameters();
                    fileDetails.recordTypes      = OPTIONS->LogfileSystemParametersRecordTypes();
                    fileDetails.recordProperties = m_Specs.SSE_SysParms_Rec;
                    fileDetails.annotations      = m_Specs.SSE_SysParms_Notes;
                ...
                case LOGFILE::BSE_DETAILED_OUTPUT: {                                                                                            // BSE_DETAILED_OUTPUT
                    ...
                    string detailedDirName = m_Config.LogBasePathString + "/" + m_Config.LogContainerName + "/" + DETAILED_OUTPUT_DIRECTORY_NAME;             // directory name with path ("/" works on Uni*x and Windows)
                    ...
                    if (detailedOutputDirectoryExists) {                                                                                        // detailed output directory exists?
                        ...
                        switch (p_Logfile) {                                                                                                    // which logfile?
                            ...
                            case LOGFILE::SSE_DETAILED_OUTPUT:                                                                                  // SSE_DETAILED_OUTPUT
                                fileDetails.recordProperties = m_Specs.SSE_Detailed_Rec;                                                              // SSE record properties
                                fileDetails.annotations      = m_Specs.SSE_Detailed_Notes;
                                break;

                            case LOGFILE::BSE_DETAILED_OUTPUT:                                                                                  // BSE_DETAILED_OUTPUT
                                fileDetails.recordProperties = m_Specs.BSE_Detailed_Rec;                                                              // BSE record properties
                                fileDetails.annotations      = m_Specs.BSE_Detailed_Notes;
                                break;
            ...
            if (!fileDetails.filename.empty() && !fileDetails.recordProperties.empty() && fileDetails.recordTypes != 0) {                       // have filename and properties?
                ...
                if (m_Config.LogfileType == LOGFILETYPE::HDF5) {                                                                                       // logging to HDF5 files?
                    ...
                    if (p_Logfile == LOGFILE::SSE_DETAILED_OUTPUT || p_Logfile == LOGFILE::BSE_DETAILED_OUTPUT) {                               // yes - detailed output file (SSE or BSE)?
                        if (m_Hdf5.DetailedId < 0) {                                                                                             // have HDF5 detailed file?
                                                                                                                                                // no - create it
                            string h5Filename = m_Config.LogBasePathString + "/" + m_Config.LogContainerName + "/" + fileDetails.filename + fileExt;          // full filename with path, container, and extension ("/" works on Uni*x and Windows)
                            ...
                            while (utils::FileExists(h5Filename)) {                                                                             // file already exists?
                                h5Filename = m_Config.LogBasePathString + "/" + m_Config.LogContainerName + "/" + fileDetails.filename + "_" + std::to_string(++version) + fileExt; // yes - add a version number and generate new filename
                            }

                            m_Hdf5.DetailedId = H5Fcreate(h5Filename.c_str(), H5F_ACC_EXCL, H5P_DEFAULT, H5P_DEFAULT);                           // create HDF5 detailed file
                            if (m_Hdf5.DetailedId < 0) {                                                                                         // created ok?                        
                ...
                if (m_Files.Enabled && ok) {                                                                                                          // logging enabled?
                ...
                if (ok) {                                                                                                                       // ok?
                                                                                                                                                // yes
                    string delimiter = "";                                                                                                      // field delimiter
                    switch (m_Files.Logfiles[fileDetails.id].filetype) {
                        case LOGFILETYPE::HDF5: delimiter = ""; break;                                                                          // HDF5
                        case LOGFILETYPE::CSV : delimiter = DELIMITERValue.at(DELIMITER::COMMA); break;                                         // CSV
                        case LOGFILETYPE::TSV : delimiter = DELIMITERValue.at(DELIMITER::TAB); break;                                           // TSV
                        case LOGFILETYPE::TXT : delimiter = DELIMITERValue.at(DELIMITER::SPACE); break;                                         // TXT
                    ...
                    m_Files.OpenStandardLogFileIds.insert({p_Logfile, fileDetails});                                                                  // record the new file details and format strings
                    ...
                    if (OPTIONS->LogfileType() == LOGFILETYPE::HDF5) {                                                                          // logging to HDF5 files?
                        for (size_t idx = 0; idx < fileDetails.hdrStrings.size(); idx++) {                                                      // for each property
                            ...
                            m_Files.Logfiles[fileDetails.id].h5File.chunkSize = chunkSize;                                                            // record chunk size for file
                            m_Files.Logfiles[fileDetails.id].h5File.IOBufSize = IOBufSize;                                                            // record IO buf size for file

                            m_Files.Logfiles[fileDetails.id].h5File.dataSets.push_back({-1, -1, TYPENAME::NONE, {}, {}});                                 // create new dataset
                            ...
                            if (h5DataType < 0) {                                                                                               // ok?
                                ...
                                m_Files.Logfiles[fileDetails.id].h5File.dataSets[idx].dataType   = fileDetails.propertyTypes[idx];                    // record COMPAS data type
                                m_Files.Logfiles[fileDetails.id].h5File.dataSets[idx].stringType = fileDetails.stringTypes[idx];                      // record COMPAS string type
                                m_Files.Logfiles[fileDetails.id].h5File.dataSets[idx].h5DataType = h5DataType;                                        // record HDF5 data type

                                // create HDF5 dataset
                                hid_t h5Dset = CreateHDF5Dataset(fileDetails.filename, 
                                                                 m_Files.Logfiles[fileDetails.id].h5File.groupId, 
                                ...
                                m_Files.Logfiles[fileDetails.id].h5File.dataSets[idx].dataSetId = h5Dset;                                             // record dataset id
                            }
                        }
                    }
                    else {                                                                                                                      // no - FS file
...
bool Log::CloseStandardFile(const LOGFILE p_Logfile, const bool p_Erase) {
    ...
    COMPASUnorderedMap<LOGFILE, LogfileDetailsT>::const_iterator logfile;                                           // iterator
    logfile = m_Files.OpenStandardLogFileIds.find(p_Logfile);                                                             // look for open logfile
    if (logfile != m_Files.OpenStandardLogFileIds.end()) {                                                                // found
        result = Close_(logfile->second.id);                                                                        // close the file
        if (result && p_Erase) {                                                                                    // closed ok and erase required?
            m_Files.OpenStandardLogFileIds.erase(logfile);                                                                // yes - remove from map
        }
    }
...
bool Log::CloseAllStandardFiles() {
    ...
    for (auto& iter: m_Files.OpenStandardLogFileIds) {                                                                    // for each open standard log file
        if (!CloseStandardFile(iter.first, false)) result = false;                                                  // close it - flag if fail
    }
    if (result) m_Files.OpenStandardLogFileIds.clear();                                                                   // remove all entries

    // close any open detailed output HDF5 container
    if (m_Hdf5.DetailedId >= 0) {                                                                                    // have open HDF5 detailed output file?
        if (H5Fclose(m_Hdf5.DetailedId) < 0) {                                                                       // yes - closed ok?
            result = false;                                                                                         // no - fail
        }
        m_Hdf5.DetailedId = -1;                                                                                      // (should have) no open HDF5 detailed output file
    }

    // close any open HDF5 container
    if (m_Hdf5.ContainerId >= 0) {                                                                                   // have open HDF5 detailed output file?
        if (H5Fclose(m_Hdf5.ContainerId) < 0) {                                                                      // yes - closed ok?
            result = false;                                                                                         // no - fail
        }
        m_Hdf5.ContainerId = -1;                                                                                     // (should have) no open HDF5 container file
    }
...
void Log::UpdateLogfileRecordSpecs(const LOGFILE             p_Logfile,
    ...
    switch (p_Logfile) {
        case LOGFILE::BSE_COMMON_ENVELOPES:
            if (p_UseDefaultProps) baseProps = m_Specs.BSE_CEE_Rec;
            baseNotes = m_Specs.BSE_CEE_Notes;
            break;
        case LOGFILE::BSE_DETAILED_OUTPUT:
            if (p_UseDefaultProps) baseProps = m_Specs.BSE_Detailed_Rec;
            baseNotes = m_Specs.BSE_Detailed_Notes;
            break;
        case LOGFILE::BSE_DOUBLE_COMPACT_OBJECTS:
            if (p_UseDefaultProps) baseProps = m_Specs.BSE_DCO_Rec;
            baseNotes = m_Specs.BSE_DCO_Notes;
            break;
        case LOGFILE::BSE_PULSAR_EVOLUTION:
            if (p_UseDefaultProps) baseProps = m_Specs.BSE_Pulsars_Rec;
            baseNotes = m_Specs.BSE_Pulsars_Notes;
            break;
        case LOGFILE::BSE_RLOF_PARAMETERS:
            if (p_UseDefaultProps) baseProps = m_Specs.BSE_RLOF_Rec;
            baseNotes = m_Specs.BSE_RLOF_Notes;
            break;
        case LOGFILE::BSE_SUPERNOVAE:
            if (p_UseDefaultProps) baseProps = m_Specs.BSE_SNE_Rec;
            baseNotes = m_Specs.BSE_SNE_Notes;
            break;
        case LOGFILE::BSE_SWITCH_LOG:
            if (p_UseDefaultProps) baseProps = m_Specs.BSE_Switch_Rec;
            baseNotes = m_Specs.BSE_Switch_Notes;
            break;
        case LOGFILE::BSE_SYSTEM_SNAPSHOT_LOG:
            if (p_UseDefaultProps) baseProps = m_Specs.BSE_Sys_Snapshot_Rec;
            baseNotes = m_Specs.BSE_Sys_Snapshot_Notes;
            break;
        case LOGFILE::BSE_SYSTEM_PARAMETERS:
            if (p_UseDefaultProps) baseProps = m_Specs.BSE_SysParms_Rec;
            baseNotes = m_Specs.BSE_SysParms_Notes;
            break;
        case LOGFILE::SSE_DETAILED_OUTPUT:
            if (p_UseDefaultProps) baseProps = m_Specs.SSE_Detailed_Rec;
            baseNotes = m_Specs.SSE_Detailed_Notes;
            break;
        case LOGFILE::SSE_SUPERNOVAE:
            if (p_UseDefaultProps) baseProps = m_Specs.SSE_SNE_Rec;
            baseNotes = m_Specs.SSE_Detailed_Notes;
            break;
        case LOGFILE::SSE_SWITCH_LOG:
            if (p_UseDefaultProps) baseProps = m_Specs.SSE_Switch_Rec;
            baseNotes = m_Specs.SSE_Switch_Notes;
            break;
        case LOGFILE::SSE_PULSAR_EVOLUTION:
            if (p_UseDefaultProps) baseProps = m_Specs.SSE_Pulsars_Rec;
            baseNotes = m_Specs.SSE_Pulsars_Notes;
            break;
        case LOGFILE::SSE_SYSTEM_SNAPSHOT_LOG:
            if (p_UseDefaultProps) baseProps = m_Specs.SSE_Sys_Snapshot_Rec;
            baseNotes = m_Specs.SSE_Sys_Snapshot_Notes;
            break;
        case LOGFILE::SSE_SYSTEM_PARAMETERS:
            if (p_UseDefaultProps) baseProps = m_Specs.SSE_SysParms_Rec;
            baseNotes = m_Specs.SSE_SysParms_Notes;
            break;
    ...
    switch (p_Logfile) {
        case LOGFILE::BSE_COMMON_ENVELOPES      : m_Specs.BSE_CEE_Rec          = newProps; m_Specs.BSE_CEE_Notes          = newNotes; break;
        case LOGFILE::BSE_DETAILED_OUTPUT       : m_Specs.BSE_Detailed_Rec     = newProps; m_Specs.BSE_Detailed_Notes     = newNotes; break;
        case LOGFILE::BSE_DOUBLE_COMPACT_OBJECTS: m_Specs.BSE_DCO_Rec          = newProps; m_Specs.BSE_DCO_Notes          = newNotes; break;
        case LOGFILE::BSE_PULSAR_EVOLUTION      : m_Specs.BSE_Pulsars_Rec      = newProps; m_Specs.BSE_Pulsars_Notes      = newNotes; break;
        case LOGFILE::BSE_RLOF_PARAMETERS       : m_Specs.BSE_RLOF_Rec         = newProps; m_Specs.BSE_RLOF_Notes         = newNotes; break;
        case LOGFILE::BSE_SUPERNOVAE            : m_Specs.BSE_SNE_Rec          = newProps; m_Specs.BSE_SNE_Notes          = newNotes; break;
        case LOGFILE::BSE_SWITCH_LOG            : m_Specs.BSE_Switch_Rec       = newProps; m_Specs.BSE_Switch_Notes       = newNotes; break;
        case LOGFILE::BSE_SYSTEM_SNAPSHOT_LOG   : m_Specs.BSE_Sys_Snapshot_Rec = newProps; m_Specs.BSE_Sys_Snapshot_Notes = newNotes; break;
        case LOGFILE::BSE_SYSTEM_PARAMETERS     : m_Specs.BSE_SysParms_Rec     = newProps; m_Specs.BSE_SysParms_Notes     = newNotes; break;
        case LOGFILE::SSE_DETAILED_OUTPUT       : m_Specs.SSE_Detailed_Rec     = newProps; m_Specs.SSE_Detailed_Notes     = newNotes; break;
        case LOGFILE::SSE_SUPERNOVAE            : m_Specs.SSE_SNE_Rec          = newProps; m_Specs.SSE_SNE_Notes          = newNotes; break;
        case LOGFILE::SSE_SWITCH_LOG            : m_Specs.SSE_Switch_Rec       = newProps; m_Specs.SSE_Switch_Notes       = newNotes; break;
        case LOGFILE::SSE_PULSAR_EVOLUTION      : m_Specs.SSE_Pulsars_Rec      = newProps; m_Specs.SSE_Pulsars_Notes      = newNotes; break;
        case LOGFILE::SSE_SYSTEM_SNAPSHOT_LOG   : m_Specs.SSE_Sys_Snapshot_Rec = newProps; m_Specs.SSE_Sys_Snapshot_Notes = newNotes; break;
        case LOGFILE::SSE_SYSTEM_PARAMETERS     : m_Specs.SSE_SysParms_Rec     = newProps; m_Specs.SSE_SysParms_Notes     = newNotes; break;
        default: break;                                                                                                 // avoids compiler warning...
    }
}
