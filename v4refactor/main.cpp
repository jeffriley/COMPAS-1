            ...
            while (!doneGridFile && evolutionStatus == EVOLUTION_STATUS::CONTINUE) {                                        // for each star to be evolved
                ...
                while (!doneGridLine && evolutionStatus == EVOLUTION_STATUS::CONTINUE) {                                    // while all ok and not done
                    ...
                    if (evolutionStatus == EVOLUTION_STATUS::CONTINUE) {                                                    // ok?
                        ...
                        // their own metallicity).  Here we use the mmetallicity as supplied by the user via the program
                        ...
                        // structure here.
                        //
                        // Here, for SSE, we only need magnitude and magnitudeRandom, and only if they were
                        // specified by the user.

                        StellarKickParmsT kickParameters;
                        if (OPTIONS->OptionSpecified("kick-magnitude"))        kickParameters.magnitude       = OPTIONS->KickMagnitude();
                        if (OPTIONS->OptionSpecified("kick-magnitude-random")) kickParameters.magnitudeRandom = OPTIONS->KickMagnitudeRandom();

                        GLOBALS->Initialise(OPTIONS->EvolutionMode());                                          // Initialise global variables (metallicity is set per-star in Star::Star)

                        STARTING_STELLAR_TYPE stellarType = OPTIONS->StellarType();
                        ...
                        star = OPTIONS->OptionSpecified("rotational-frequency")                                             // user specified rotational frequency?
                                ? new Star(stellarType, randomSeed, metallicity, mass, kickParameters, OPTIONS->RotationalFrequency())
                                : new Star(stellarType, randomSeed, metallicity, mass, kickParameters);
                        ...
                        if (!OPTIONS->Quiet()) {                                                                            // quiet mode?
                            SAY(index                                       <<                                                // announce result of evolving the star
                                ...
                                ": "                                        <<
                                STARTING_STELLAR_TYPE_LABEL.at(stellarType) <<              
                                ", RandomSeed = "                           <<
                                randomSeed                                  <<
                                ", Mass = "                                 <<
                                mass                                        <<
...
int main(int argc, char * argv[]) {
    ...
    RAND->Initialise();                                                                             // Initialise the random number generator
    ...
    if (!ok) {                                                                                      // have commandline options ok?
        ...
        if (OPTIONS->RequestedHelp()) {                                                             // user requested help?
            (void)utils::SplashScreen(true);                                                        // yes - show splash screen
        ...
        if (programStatus == PROGRAM_STATUS::CONTINUE) {
            ...
            GLOBALS->Initialise(OPTIONS->EvolutionMode());                                          // initialise the globals functionality

            ZDEP->Initialise(OPTIONS->MetallicityCacheSize());                                // Initialise Z-dependent cache
            ...
            (void)utils::SplashScreen(true);                                                            // announce ourselves
            ...
            if (!LOGGING->Enabled()) programStatus = PROGRAM_STATUS::LOGGING_FAILED;                // logging failed to start
                ...
                if (programStatus == PROGRAM_STATUS::CONTINUE) {                                    // all ok?
                    ...
                    if (OPTIONS->EvolutionMode() == EVOLUTION_MODE::SSE_HURLEY) {                          // SSE?
