...
// error_helpers: implementation namespace for the SHOW_ERROR / THROW_ERROR /
// SHOW_WARN macro families defined in ErrorsMacros.h. Each macro is now a
// thin one-line wrapper over a function call here, replacing the previous
// _0/_1/_2/_3/_4/_5 + GET_MACRO dispatcher pattern (87 macros down to 12).
//
// Each function has two overloads: a 5-arg form (no qualifying string) and a
// 6-arg form (with qualifying string). The macros use __VA_OPT__ to pass or
// not-pass the optional qualifying string; overload resolution picks the
// matching function.
//
// The instance-bound macros (SHOW_ERROR, ...) capture the calling object's
// ObjectId() / ObjectType() / StellarType() at the call site. The _STATIC
// variants pass hardcoded -1L / OBJECT_TYPE::NONE / STELLAR_TYPE::NONE for
// use outside member-function context (e.g. main, free functions, statics).
namespace error_helpers {

    // SHOW_ERROR family - print "ERROR: ..." without throwing.
    inline void show_error(ERROR e,
                           OBJECT_ID id, OBJECT_TYPE ot, STELLAR_TYPE st,
                           const char* fn) {
        Errors::Instance()->ShowIt(ERROR_PREFIX, e, "", id, ot, st, fn);
    }
    inline void show_error(ERROR e, const std::string& qual,
                           OBJECT_ID id, OBJECT_TYPE ot, STELLAR_TYPE st,
                           const char* fn) {
        Errors::Instance()->ShowIt(ERROR_PREFIX, e, qual, id, ot, st, fn);
    }

    // SHOW_WARN family - print "WARNING: ..." without throwing.
    inline void show_warn(ERROR e,
                          OBJECT_ID id, OBJECT_TYPE ot, STELLAR_TYPE st,
                          const char* fn) {
        Errors::Instance()->ShowIt(WARNING_PREFIX, e, "", id, ot, st, fn);
    }
    inline void show_warn(ERROR e, const std::string& qual,
                          OBJECT_ID id, OBJECT_TYPE ot, STELLAR_TYPE st,
                          const char* fn) {
        Errors::Instance()->ShowIt(WARNING_PREFIX, e, qual, id, ot, st, fn);
    }

    // THROW_ERROR family - print "ERROR: ..." then throw the error number.
    // Marked [[noreturn]] so the compiler knows control doesn't return,
    // matching what the previous macro form provided implicitly via the
    // literal `throw` at the call site.
    [[noreturn]] inline void throw_error(ERROR e,
                                         OBJECT_ID id, OBJECT_TYPE ot, STELLAR_TYPE st,
                                         const char* fn) {
        show_error(e, id, ot, st, fn);
        throw static_cast<int>(e);
    }
    [[noreturn]] inline void throw_error(ERROR e, const std::string& qual,
                                         OBJECT_ID id, OBJECT_TYPE ot, STELLAR_TYPE st,
                                         const char* fn) {
        show_error(e, qual, id, ot, st, fn);
        throw static_cast<int>(e);
    }
