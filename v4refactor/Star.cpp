...
Star::Star(const unsigned long int     p_RandomSeed,
    ...
    m_ObjectId          = GLOBALS->NextObjectId();                                                          // Set object id
    ...
    if (utils::IsOneOf(startingStellarType, MAIN_SEQUENCE)) {                                               // Starting stellar type MS (won't be CH here)?
                                                                                                            // Yes
        // Set main sequence type based on ZAMS mass
        startingStellarType = p_Mass <= 0.7 ? STELLAR_TYPE::MS_LTE_07 : STELLAR_TYPE::MS_GT_07;
    }

    // Construct a BaseStar object  
    m_Star = new BaseStar(this, p_RandomSeed, startingStellarType, p_Metallicity, p_Mass, p_KickParameters, p_RotationalFrequency); // Create underlying BaseStar object

    (void)SwitchTo(startingStellarType, true, false);                                                       // Switch to correct starting stellar type, but don't log the switch
    ...
    if (utils::IsOneOf(startingStellarType, MAIN_SEQUENCE)) {                                               // Starting stellar type MS (won't be CH here)?
        ...
        if (OPTIONS->CHEMode() != CHE_MODE::NONE) {                                                         // Yes - CHE enabled?
                                                                                                            // Yes           
            if (omega >= utils::CalculateMinOmegaForCHE_Butler2018(p_Metallicity, p_Mass)) {                // Spinning fast enough for CHE?
                (void)SwitchTo(STELLAR_TYPE::CHEMICALLY_HOMOGENEOUS, true, false);                          // Yes - switch, but don't log the switch
            }
        }
    }

    // Thresholds flags for system snapshot output file   <<<<<<<<<<<<<<<<<<<<<<<<< JR FIX THIS - WHY ARE THESE HERE?????????????????
...
Star::Star(const Star& p_Star) {
    if (p_Star.m_Star) {                                    // p_Star.m_Star exists?
                                                            // Yes
        ...
        m_Star = std::make_unique<decltype(p_Star.m_Star)>(*(p_Star.m_Star), false);
    }
    else {                                                  // No - p_Star.m_Star does not exist
        THROW_ERROR(ERROR::NO_OBJECT);                      // Throw error
    }
}
 ...
 *
 * @brief
 * Clone the star passed as p_Star, and switch the clone to the specified stellar type.
 *
 * Instantiates new object of specified class (stellar type), and returns a std::unique_ptr to
 * the caller.  The caller does not need to return any memory allocated - the std::unique_ptr
 * is automatically destroyed (and memory returned) when it goes out of the caller's scope.
 * 
 * The clone will be an exact copy of p_Star, except:
 * 
 *    - the stellar type will be per the p_StellarType parameter
 *    - the object persistence will be set per the p_Persistence parameter
 *    - the clone may be initialised, per the p_Initialise parameter
 * 
 *
 * std::unique_ptr<BaseStar> Clone(const std::unique_ptr<BaseStar> p_Star, const STELLAR_TYPE p_StellarType, const OBJECT_PERSISTENCE p_Persistence, const bool p_Initialise) const
 *
 * @param       p_Star                          The star to be clone
 ...
 */
std::unique_ptr<BaseStar> Star::Clone(const std::unique_ptr<BaseStar> p_Star, const STELLAR_TYPE p_StellarType, const OBJECT_PERSISTENCE p_Persistence, const bool p_Initialise) const {

    std::unique_ptr<BaseStar> clone;

    switch (p_StellarType) {
        case STELLAR_TYPE::MS_LTE_07                                : {clone = std::make_unique<MS_lte_07>(*p_Star, p_Initialise);} break;
        case STELLAR_TYPE::MS_GT_07                                 : {clone = std::make_unique<MS_gt_07>(*p_Star, p_Initialise);} break;
        case STELLAR_TYPE::CHEMICALLY_HOMOGENEOUS                   : {clone = std::make_unique<CH>(*p_Star, p_Initialise);} break;
        case STELLAR_TYPE::HERTZSPRUNG_GAP                          : {clone = std::make_unique<HG>(*p_Star, p_Initialise);} break;
        case STELLAR_TYPE::FIRST_GIANT_BRANCH                       : {clone = std::make_unique<FGB>(*p_Star, p_Initialise);} break;
        case STELLAR_TYPE::CORE_HELIUM_BURNING                      : {clone = std::make_unique<CHeB>(*p_Star, p_Initialise);} break;
        case STELLAR_TYPE::EARLY_ASYMPTOTIC_GIANT_BRANCH            : {clone = std::make_unique<EAGB>(*p_Star, p_Initialise);} break;
        case STELLAR_TYPE::THERMALLY_PULSING_ASYMPTOTIC_GIANT_BRANCH: {clone = std::make_unique<TPAGB>(*p_Star, p_Initialise);} break;
        case STELLAR_TYPE::NAKED_HELIUM_STAR_MS                     : {clone = std::make_unique<HeMS>(*p_Star, p_Initialise);} break;
        case STELLAR_TYPE::NAKED_HELIUM_STAR_HERTZSPRUNG_GAP        : {clone = std::make_unique<HeHG>(*p_Star, p_Initialise);} break;
        case STELLAR_TYPE::NAKED_HELIUM_STAR_GIANT_BRANCH           : {clone = std::make_unique<HeGB>(*p_Star, p_Initialise);} break;
        case STELLAR_TYPE::HELIUM_WHITE_DWARF                       : {clone = std::make_unique<HeWD>(*p_Star, p_Initialise);} break;
        case STELLAR_TYPE::CARBON_OXYGEN_WHITE_DWARF                : {clone = std::make_unique<COWD>(*p_Star, p_Initialise);} break;
        case STELLAR_TYPE::OXYGEN_NEON_WHITE_DWARF                  : {clone = std::make_unique<ONeWD>(*p_Star, p_Initialise);} break;
        case STELLAR_TYPE::NEUTRON_STAR                             : {clone = std::make_unique<NS>(*p_Star, p_Initialise);} break;
        case STELLAR_TYPE::BLACK_HOLE                               : {clone = std::make_unique<BH>(*p_Star, p_Initialise);} break;
        case STELLAR_TYPE::MASSLESS_REMNANT                         : {clone = std::make_unique<MR>(*p_Star, p_Initialise);} break;
    ...
    clone->SetStellarType(p_StellarType);                   // Set new stellar type
    clone->SetPersistence(p_Persistence);                   // Set object persistence
 ...
 * switch the current star to a new stellar type, we just want to know what would happen if we
 * did.  The solution in these cases is to call a class member function for the stellar type in
 * question, but in many cases we don't have an instantiated object of the required class 
 * (stellar type).  The best solution is to call a static function of the class required, but
 * sometimes exposing the required static function is impractical - the function may call other
 * functions, which in turn may call other functions, all of which would have to be exposed as
 * static functions.  Where exposing a  static function is not practical, cloning the current
 * star and switching the clone to the new stellar type is a good solution - the clone can be
 * discarded once the attribute checks are made.
 * 
 * There is some overhead in instantiating the clone object, so the preferred solution is a static
 * function whenever possible - use this functionality sparingly, and only when necessary.
 * 
 * See also Star::MakeStar().
 *
 *
 * std::unique_ptr<BaseStar> CloneAs(const STELLAR_TYPE p_StellarType, const OBJECT_PERSISTENCE p_Persistence, const bool p_Initialise) const
 *
 * @param       p_StellarType                   Resultant stellar type of the clone
 * @param       p_Persistence                   Object persistence ({OBJECT_PERSISTENCE::PERMAMNENT, OBJECT_PERSISTENCE::EPHEMERAL})
 * @param       p_Initialise                    Flag to indicate whether the clone star should be initialised (default is TRUE) (bool)
 * @return                                      Pointer to the cloned star (std::unique_ptr<BaseStar>)
 */
std::unique_ptr<BaseStar> Star::CloneAs(const STELLAR_TYPE p_StellarType, const OBJECT_PERSISTENCE p_Persistence, const bool p_Initialise) const {
    return Clone(m_Star, p_StellarType, p_Persistence, p_Initialise);
}
 ...
 *
 * @brief
 * Make a new (unevolved) star of the specified stellar type.
 *
 * Instantiates new object of specified class (stellar type), and returns a std::unique_ptr to
 * the caller.  The caller does not need to return any memory allocated - the std::unique_ptr
 * is automatically destroyed (and memory returned) when it goes out of the caller's scope.
 * 
 * The new star will be initialised.
 * Sets stellar type.
 * Sets object persistence (PERMANENT or EPHEMERAL).
 * 
 * Occasionally during the evolution of a star or binary we want to check an attribute of a star
 * of a particular stellar type - we don't actually want to switch the current star to a new 
 * stellar type, we just want to know what would happen if we had a star of a particular stellar
 * type.  The solution in these cases might be to call a class member function for the stellar
 * type in question, but in many cases we don't have an instantiated object of the required class
 * (stellar type) - and even if we did, it would likely be an evolved star.  The best solution is
 * to call a static function of the class required, but sometimes exposing the required static
 * function is impractical - the function may call other functions, which in turn may call other
 * functions, all of which would have to be exposed as static functions.  Where exposing a static
 * function is not practical, creating a new (unevolved) star of the required stellar type is a
 * good solution - the new star can be discarded once the attribute checks are made.
 * 
 * There is some overhead in instantiating the new object, so the preferred solution is a static
 * function whenever possible - use this functionality sparingly, and only when necessary.
 * 
 * See also Star::CloneAs().
 * 
 *
 * std::unique_ptr<BaseStar> MakeStar(const STELLAR_TYPE p_StellarType, const OBJECT_PERSISTENCE p_Persistencee) const
 *
 * @param       p_StellarType                   Resultant stellar type of the clone
 * @param       p_Persistence                   Object persistence ({OBJECT_PERSISTENCE::PERMAMNENT, OBJECT_PERSISTENCE::EPHEMERAL})
 * @return                                      Pointer to the new star (std::unique_ptr<BaseStar>)
 */
std::unique_ptr<BaseStar> Star::MakeStar(const STELLAR_TYPE p_StellarType, const OBJECT_PERSISTENCE p_Persistence) const {
    std::unique_ptr<BaseStar> star;
    return Clone(star, p_StellarType, p_Persistence);
}
 ...
 *
 * @brief
 * Switch current star to required stellar type.
 *
 * Star is cloned, and initialised, as new stellar type.
 * Stellar type is set to new stellar type.
 * Object persistence is copied (unchanged).
 ...
 * @param       p_Log                           Flag to indicate whether the switch should be logged in the SwitchLog file (default is true)
 * @return                                      Stellar type of star before switch (previous stellar type)
 */
STELLAR_TYPE Star::SwitchTo(const STELLAR_TYPE p_StellarType, bool p_SetStartingType, bool p_Log) {

    STELLAR_TYPE stellarTypePrev = StellarType();

    // Only switch if p_StellarType != current stellar type
    if (p_StellarType != StellarType()) {

        m_Star = Clone(m_Star, p_StellarType, m_Star->ObjectPersistence()); // Clone as new stellar type
        ...
        // Log only if star is switching from one of the evolvable types.
        if (OPTIONS->SwitchLog() && !p_NoLog && utils::IsOneOf(stellarTypePrev, EVOLVABLE_TYPES)) {
            ...
            if (OPTIONS->EvolutionMode() == EVOLUTION_MODE::BSE_HURLEY) {   // BSE?
...
STELLAR_TYPE Star::UpdateAttributesAndAgeOneTimestep(const double p_DeltaMass,
    ...
    // Fork-B adapter: route to AdvanceOneTimestep. The mass deltas are externally
    // supplied (BSE mass transfer); the timestep is also externally supplied.
    //
    // p_Switch and p_ForceRecalculate are accepted for signature compatibility with
    // Phase-5 BSE callers but are no-ops in the unified model: AdvanceOneTimestep
    // always recomputes fresh (force is implicit) and always returns the next stellar
    // type leaving the switch decision to the caller (the commented-out switch block
    // below preserves the CH->NakedHeMS edge case for future revisit).
    (void)p_Switch;
    (void)p_ForceRecalculate;
    STELLAR_TYPE stellarType = m_Star->AdvanceOneTimestep(p_DeltaTime, p_DeltaMass, p_DeltaMass0);
 ...
 *
 *
 * See BaseStar::AdvanceOneTimestep() for details of operation.
 *
...
STELLAR_TYPE Star::UpdateAttributes(const double p_DeltaMass, const double p_DeltaMass0, const bool p_ForceRecalculate) {
    // Fork-B adapter: explicit dt=0 means "apply mass change(s), recompute attributes,
    // no time advance" - the BSE mass-only path. p_ForceRecalculate accepted for
    // signature compatibility; AdvanceOneTimestep always recomputes fresh.
    (void)p_ForceRecalculate;
    return m_Star->AdvanceOneTimestep(0.0, p_DeltaMass, p_DeltaMass0);
}
...
STELLAR_TYPE Star::EvolveOneTimestep(const double p_dt) {
    ...
    STELLAR_TYPE nextStellarType = m_Star->AdvanceOneTimestep(p_dt);
...
EVOLUTION_STATUS Star::Evolve(const long int p_Id) {
    ...
    try {
        ...
        m_Id = p_Id; // store the id  <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< in state??????  JR FIX THIS - SHOULD BE FIXED ATTRIBUTES

        // evolve the star

//// NO - DO IT AT START OF TIMESTEP <<<<<<<<<<<<        m_Star->CalculateGBParams(); // calculate giant branch parameters - should be done initially, then at state change <<<<<<<<<<<<<<<<<<<<<


        double dt;
        [[maybe_unused]] double dtPrev;
        ...
        while (evolutionStatus == EVOLUTION_STATUS::CONTINUE) {                                                 // timestep loop
            ...
            if (m_Star->Time() > OPTIONS->MaxEvolutionTime()) {                                                 // out of time?
            ...
            else if (utils::IsOneOf(StellarType(), WHITE_DWARFS) || StellarType() == STELLAR_TYPE::BLACK_HOLE) {// WD or BH?
                ...
                // JR - where to put enevlope loss here?  Different from stellar type change?  Envelope loss may mean new age, mass, etc. - check this   <<<<<<<<<< JR FIX THIS
                ...
                if (IsSupernova()) {                                                                            // is star about to go supernova?
                    ...
                    if (nextStellarType != StellarType()) {                                                     // stellar type change?
                        ...
                        if (OPTIONS->EvolutionMode() == EVOLUTION_MODE::SSE_HURLEY && m_ObjectPersistence == OBJECT_PERSISTENCE::PERMANENT) {
                            m_Star->PrintSupernovaDetails();
                        }
                    }
                    else {
                        // THIS IS AN ERROR!  JR FIX THIS <<<<<<<<<<<<<<<<<<<<<<<<
                    }
                }
                ...
                // process ordinary timestep
                    ...
                    nextStellarType = EvolveOneTimestep(dt);                                                        // evolve for timestep (single per-step entry: AdvanceOneTimestep)
